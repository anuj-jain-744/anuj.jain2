...
jest.mock("../../components/Accordians", () => ({
  Accordians: jest.fn(({ content }: { content: { qns: string; ans: string }[] }) => (
    <div>
      {content.map((item: { qns: string; ans: string }, index: number) => (
        <div key={index}>
          <h2>{item.qns}</h2>
          <p>{item.ans}</p>
        </div>
...
describe("FAQShared Component", () => {
  ...
  it("renders the component correctly", () => {
    const { getByText } = render(
      <FAQShared faqCont={faqCont} categories={categories} accordianData={accordianData} />
    );
  ...
  it("does not render accordions if there are no categories", () => {
    const { container } = render(
      <FAQShared faqCont={faqCont} categories={[]} accordianData={accordianData} />
    );
