  ...
  return (
    showButtons && (
      <div className="carousel-button-group">
        ...
        >
          {!page ? (<KeyboardArrowLeftIcon
            fontSize="small"
            className={`${styles.arrowprev} ${
              currentSlide === 0 ? "ondisable" : ""
            }`}
        ...
        <Button
          ...
          className={`carousel-button-next ${totalItems && currentSlide === totalItems - maxSlides
              ? "disable"
              : ""
            }`}
          onClick={() => {
        ...
        >
          {!page ? (<KeyboardArrowRightIcon
            fontSize="small"
            className={`${styles.arrownext} ${totalItems && currentSlide === totalItems - maxSlides
                ? "ondisable"
                : ""
              }`}
      ...
      <animated.div style={isVisible ? springServices : {}} className="tab-buttons">
        ...
        {hideProductTab && (
          <div className="parent-menu">
            {tabContents.length > 0 && tabContents.map((tab, index) => (
              <div
                data-testid={`parentTab-${index}`}
                key={index}
                // Do not remove: in future sprint we would need to enable toggle where this onClick will be utilized
                // onClick={() => {
                //   if (tab.title !== "Quick Renewal") {
                //     handleParentTabClick(index);
                //   }
                // }}
                className={`${
                  activeParentTabIndex === index ? "active" : ""
                } tab-list walaa-medium-500 ${
                  tab.title === "Quick Renewal" ? "disabled" : ""
                }`}
              >
                {tab.title}
              </div>
