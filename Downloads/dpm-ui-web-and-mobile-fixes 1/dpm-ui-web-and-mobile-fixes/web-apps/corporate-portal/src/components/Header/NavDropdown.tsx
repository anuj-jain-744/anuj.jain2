...
import { OrgChevronDown, OrgChevronUp } from "../../assets/Header";
...
> = ({
  ...
  return (
    <NavDropdown
      ...
      key={pIdx}
      onMouseEnter={() =>
        handleDropdownEvent(parent?.linkName ?? "", true, pIdx)
      }
      onMouseLeave={() => handleDropdownEvent("", false, 0)}
      className={`${
        dropdownStates?.linkName === parent?.linkName ? "active" : ""
      } ${navbarTransparent ? "theme-transparent" : "theme-original"}`}
      onClick={() => parent?.menuUrl && navigateTo && navigateTo(parent?.menuUrl) }
    >
