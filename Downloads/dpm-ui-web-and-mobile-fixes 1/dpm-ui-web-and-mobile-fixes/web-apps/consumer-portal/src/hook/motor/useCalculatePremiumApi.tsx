...
export function useCalculatePremiumApi() {
  ...
  useEffect(() => {
    ...
    const errorMessages = Object.entries(errors)
      ...
      .map(([api, error]) => {
        const errorMessage = (error as ErrorResponse).messages.message_en;
  ...
  useEffect(() => {
    ...
    if (dataPricingOptions) {
      ...
      if (pricingOptions) {
        const amounts = pricingOptions.map(option => option.deductibleAmount).sort((a, b) => a - b);
...
export function mockReturnValue(mockUseCalculatePremiumApi: { handleCalculatePremium: jest.Mock<any, any, any>; isAllError: boolean; isLoadingCalculatePremium: boolean; isCalculateData: boolean; isError: undefined; }) {
