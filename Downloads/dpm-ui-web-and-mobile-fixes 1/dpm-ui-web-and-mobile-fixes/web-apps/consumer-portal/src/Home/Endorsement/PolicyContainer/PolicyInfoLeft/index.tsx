...
const PolicyInfoLeft: FC<PolicyCardProps> = ({ policyDetails, languageData, handleSelectAddBenefits, setIsTermCondition }) => {
  ...
  const endorsements = ['Add Benefits', 'Add Beneficiary']

  const benefits = viewPolicy?.policyLob[0]?.policyRisk[0]?.policyCoverage || [];
  ...
  return (
    <div className="policy-claim-left-card">
      {viewPolicy ? <div className="policy-claim-container">
        ...
        <div className="endorsement-box-container">
          {endorsements.map((item, index) => {
            ...
            return (<button key={index} className={`endorsement-box-wrapper ${selectedEndorsement === item ? 'selected' : ''}`} onClick={handleOnChange(item)}>
      ...
      {(viewPolicy && selectedEndorsement === 'Add Benefits') &&
        <>
          <AddBenefits languageData={languageData} viewPolicy={viewPolicy} isBenefitLoaded={isBenefitLoaded} benefitsClaimed={benefitsClaimed} benefitsData={benefits} handleSelectAddBenefits={handleSelectAddBenefits} />
          <TermsAndCon
            languageData={languageData}
