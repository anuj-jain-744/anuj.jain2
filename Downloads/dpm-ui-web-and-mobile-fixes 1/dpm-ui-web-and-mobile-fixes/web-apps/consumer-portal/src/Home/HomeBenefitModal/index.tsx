        ...
        <div className="sub-sub-heading">
          ...
          <div className="repair-right-side">
            ...
            <div className="sar-value">
              {languageData?.sar} {" "}
              <span className="price1">{price1}</span>
            </div>
          </div>
          ...
          <div className="repair-right-side1">
            ...
            <div className="sar-value">
              {languageData?.sar} {" "}
              <span className="price1">{price2}</span>
            </div>
          </div>
        </div>
            ...
            typeof item.type === 'object' ? (
              item.key && openAccordion.indexOf(item.key) !== -1 ?
                item.type.map((subitems, index) => (
                  <CoverageItem key={index} triggerViewBenefit={triggerViewBenefit} items={subitems} />
                )) : '')
              : (
                <CoverageItem key={index} triggerViewBenefit={triggerViewBenefit} items={item} />
              )))}
        </div>
      </div>
    </Modal>
