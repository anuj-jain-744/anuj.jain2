...
import { HOME_SCHEME_CODE, Chargeable } from 'constant';
...
const contentBenefitsDeclaration = (policyCoverage, languageData: LanguageData) => {
...
const additionalBenefits = (policyCoverage, languageData: LanguageData) => {
  ...
  const mappedBenefits = benefits?.filter(item => item?.benefitCategory === Chargeable && item?.premiumInfo?.finalPremium > 0).
...
const renewalPayloadRequest = (data: { [key: string]: string }, languageData: LanguageData) => {
  ...
  const policyCoverage = policyRisk.policyCoverage;
  const newPolicyCoverage = [];
  newPolicyCoverage?.push({
    coverageCode: languageData?.defaultCoverageCode,
    declarationItem: contentBenefitsDeclaration(policyCoverage, languageData)
  });
  const additionalData: { coverageCode: string }[] = additionalBenefits(policyCoverage, languageData);
  newPolicyCoverage?.concat(additionalData);

  return {
    schemeCode: policyBasic?.schemeCode,
    planCode: 4,
    ownerOrTenant: policyLob?.ownerOrTenant,
    effectiveDate: apiFormatDate(currrentDate, '/'),
    ...
    policyRisk: [{
      policyCoverage: newPolicyCoverage,
...
const updateFormAddressSelection = (formAddressSelection: TyperFormAddressSelection, userInfo: UserInfo, showPropertyMap: PropertyMap, languageData: LanguageData) => {
  ...
  return {
    ...
    //cityDistrict: address.cityENG, // Waiting from API data.
...
function usePropertyPayload() {
  ...
  const updateDeclarationItems = (payload, declartion, declare, riskID) => {
    let changedDeclarationItems: { [key: string]: number } = {};
  ...
  return { init, getRiskID, userAddressFormSelection, updateDeclarationItems, renewalPayloadRequest, contentBenefitsDeclaration }
