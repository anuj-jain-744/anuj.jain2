...
jest.mock('Motor/Policy-services/PolicyDashboard/hooks/useReviewPolicy', () => ({
  useReviewPolicy: jest.fn().mockReturnValue({ makeApiCall: jest.fn(),data: jest.fn(),isLoading:false }),
}));
...
jest.mock("./registerClaimCard", () => jest.fn(() => <div>RegisterClaimCard</div>));
jest.mock("./CancelSuccessLeft", () => jest.fn(() => <div data-testid="cancel-successid">CancelSuccessLeft</div>));
jest.mock("./CommonComponent/ContactCard", () => jest.fn(()=><div data-testid="contact-card">ContactCard</div>));
jest.mock("./CommonComponent/PolicyDetails", () => jest.fn(()=><div data-testid="policy-details">PolicyDetails</div>));
jest.mock("../../components/Feedback", () => jest.fn(()=><div data-testid="feedback">Feedback</div>));
jest.mock("./CommonComponent/UserCard", () => jest.fn(()=><div data-testid="user-card">UserCard</div>));
jest.mock("./CommonComponent/VehicleInfo", () => jest.fn(()=><div data-testid="vehicle-info">VehicleInfo</div>));
jest.mock("./CommonComponent/ActionLinks", () => ({
  ActionLInk: ({
    ...
    languageData && <div data-testid="action-links" onClick={handleDownloadPolicy}>
      {languageData.download_payment_receipt}
    </div>
  ),
}));
jest.mock('components/AlertBox', () => ({
  AlertBox: ({ title, description, showAlertModal }:{title:string,description:string,showAlertModal:boolean}) => 
...
describe("LeftSuccess Component", () => {
  ...
  it("should render CancelSuccessLeft correctly", () => {
    ...
    render(
      ...
      <LeftSuccess
          data={{ policyNumber: "123", vehicleDetails: { name: "Car", plateNumber: "XYZ123", vehicleSequenceNo: "001", chassisNo: "123456", manufactureYear: 2020 }, refundValue: "100" }}
          status={true}
  ...
  it("should render the PolicyDetails ", async () => {
    const mockApiResponse = { data: { model: { policyId: 'policy123',policyNo:"123" } } };
    ...
    render(
      <Provider store={mockStore}>
        <LeftSuccess
          data={{
            policyNumber: "123",
            vehicleDetails: { name: "Car", plateNumber: "XYZ123", vehicleSequenceNo: "001", chassisNo: "123456", manufactureYear: 2020 },
            refundValue: "100",
          }}
          status={true}
          setLoading={mockSetLoading}
