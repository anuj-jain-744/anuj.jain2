...
interface HomeInfoSuccessProps {
  policyData: {policyHolderDetails: {address: {
    buildingNumber: string, city: string, cityInArabic: string, district: string, streetName: string,
    streetNameInArabic: string, districtNameInArabic: string, postCode: string
  }}, 
  planDetails: { planCode: string}} | null;
}
...
const HomeInfoSuccess: React.FC<HomeInfoSuccessProps> = ({
  ...
  const address = policyData?.policyHolderDetails?.address;
  ...
  const coverageValue = policyData?.planDetails?.planCode?.replace(/\s+/g, '').toLowerCase() || null;
