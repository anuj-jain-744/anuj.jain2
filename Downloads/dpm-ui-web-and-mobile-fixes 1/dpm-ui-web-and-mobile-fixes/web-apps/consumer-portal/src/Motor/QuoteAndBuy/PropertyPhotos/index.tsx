...
import useFilesToBase64 from "../../../../src/claims/register/TrackClaim/hooks/useFilesToBase64";
import FileList from "./FileList";
import { validatePropertyPhotos } from "utils/fileValidation";
...
const PropertyPhotosModal: React.FC<PropertyPhotosProps> = ({
  ...
  const handleDrop = (event: React.DragEvent<HTMLDivElement>) => {
    ...
    const isValidDrop = validatePropertyPhotos(
      droppedFiles,
      languageData ?? { file_size_error: "Only JPG files under 800 KB are allowed." }
    );
    if (!isValidDrop) {
      showFileSizeError(languageData?.file_size_error || "Only JPG files under 800 KB are allowed.");
      return;
    }
  ...
  return (
    <Card className="property-photos-container">
      ...
      >
        <Modal.Header closeButton>{languageData?.guidelines_title}</Modal.Header>
      ...
      <div className="content-description"><InfoOutlinedIcon /> {propsData?.ownerDetail?.ownerFullNameEnglish}, {languageData?.property_photos_text}</div>
