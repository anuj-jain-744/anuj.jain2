...
export default function PaymentInsurance({ transactionId }: Readonly<PaymentInsuranceProps>) {
  ...
  const productCode = location.state?.productCode ?? processTransaction?.productCode ?? "";
  ...
  if(productCode === PRODUCTCODE_HOME) { // need to add admin fess with plan amount for home
    ...
    paymentInfo = {
      adminFees: premiumInfo?.taxFeeBreakdowns[1]?.amount,
  ...
  return (
    <div className="payment-insurance">
      <AlertBox 
        title="Payment"
        description={processTransaction?.errorMessage?.split("-").join(" ") ?? ""}
      ...
      <div className="payment-buy-container" data-testid="quote-buy">
          ...
          <div className="payment-options">
            {paymentOptions.map((option, index) => (
              ...
              >
                ...
                {cmsLang && quoteDataValue &&  paymentData && option.value === selectedPaymentOption && (
                  <PaymentOptions 
                    option={option.value} 
        ...
        <div className="rightPanel">
          <QuoteCreation quotationNum={propsData?.quoteNo} />
