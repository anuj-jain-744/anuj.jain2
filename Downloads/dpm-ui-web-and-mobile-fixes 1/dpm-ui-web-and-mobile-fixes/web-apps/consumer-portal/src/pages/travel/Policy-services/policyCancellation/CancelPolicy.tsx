...
import { SA, ibanLength, TRAVEL_POLICY_TYPE, TRAVEL_TYPE, Home_POLICY_TYPE, HOME } from "constant";
...
const [fileData, setFileData] = useState<(FileData | null)[]>([
  ...
  const { reviewPolicy, setReviewPolicy, setRefundPolicy, refundPolicy } =
    ...
    const { fetchCancelPolicy,
    isLoading: submitLoading,
    error: submitCancelPolicyError,
    policyCancelData: policySubmit, } = useCancelPolicy({
    PolicyNo: selectedPolicyNumber!,
    documents: fileData
  ...
  const cmsReasons = languageData?.select_dropdown_travel.split(",");
  const dropdownItems =
    cmsReasons &&
    cmsReasons.map((item: string, index: number) => {
      return { value: (index + 8).toString(), item: item };
    });
  ...
  const handleSuccessValidation = async() => {
    ...
    await fetchCancelPolicy(reason);
    if (isTCAccepted) {
  ...
  const policyStartDate = policyDetails ? formatDate(policyDetails?.startDate) : "";
  const policyEndDate = policyDetails ? formatDate(policyDetails?.expiryDate) : ""; 
  
  const sumInsuredDeductible = viewPolicyData?.policyLob[0]?.policyRisk && viewPolicyData?.policyLob[0].policyRisk[0]?.policyCoverage?.find(
    (item: { sumInsured: any; minDeductible: any }) => item.sumInsured !== null && item.minDeductible !== null
  );

  setReviewPolicy(viewPolicyData?.model);
  useEffect(() => {
    if (policyDetails) {
  ...
  const refundValue = policyDataObj?.productCode === HOME ? policyData?.data?.cancelRefund : policyData?.data?.model?.cancelRefund;

  const isrefundSectionShow = refundValue === 0 || refundValue === undefined || refundValue === null
  ? false : true;

  const formattedRefundValue =
  refundValue === 0 || refundValue === undefined || refundValue === null
    ? noRefund
    : `${priceUnit} ` + Math.abs(refundValue).toFixed(2);
  ...
  const policydetail = {
    ...
    coverageType: policyDataObj?.productCode === HOME ? Home_POLICY_TYPE[Number(policyDetails?.coverageName)] : TRAVEL_POLICY_TYPE[Number(policyDetails?.coverageName)],
    productCode : policyDataObj?.productCode,
  };
  useEffect(() => {
    
    setRefundPolicy(policyDataObj?.productCode === HOME ? policydetail: policyData?.data?.model);
  }, [policyData]);
  ...
  const isSubmitDisabled =
        ...
        !isTCAccepted ||
        !isValidIban ||
        (!iban.startsWith(SA) && (!isReEnterIbanValid || !isBankNameValid || bankName.trim() === "")) ||
        (!iban.startsWith(SA) && !chequeLeafUploaded) : !isTCAccepted;

        if (submitLoading || fetchCancelRefundLoading) {
  ...
  return (
    ...
    <AlertBox
                      ...
                      <div className="body-content-parent">
                        ...
                        {isFlagged && isCancelAnyway && (
                          <div className="body-content">
                            ...
                            {(formattedRefundValue === noRefund) ? (<SeniorCitizenAlert message={languageData?.no_refund_applicable_msg} />): (<div className="refund-tooltip">
            ...
            <div className="right-card-policy">
              {selectedPolicyNumber && (
                <PolicyCard
                  ...
                  policyHolder = {policyDataDetail?.policyHolderDetails?.customerNameEnglish}
                  sumInsured = {sumInsuredDeductible?.sumInsured}
                  deductibles = {sumInsuredDeductible?.minDeductible}
                />
              )}
              <DidYouKnowCard did_you_know_content={travelData?.did_you_know_content} 
      ...
      {isCancelPolicySuccess && (
            <PHQuoteBuyProvider>
          <QuoteAndBuyProvider>
            <TravelSucces status={false} data={travelData} flag={false} />
          </QuoteAndBuyProvider>
        </PHQuoteBuyProvider>
      )}
      ...
      {languageData && (
        <OTPWrapper
          ...
          payload={{
            mobileNumber: policyDataObj?.productCode === HOME ? propsData?.mobileNo : propsData?.mobileNumber,
          }}
