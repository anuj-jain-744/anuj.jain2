...
const QuoteCards: React.FC<QuoteCardsProps> = ({  languageData, setLeftStep, leftStep }) => {
  ...
  // For travel card
  useEffect(() => {
    if(travelcoverage && travelcoveragePlan) {
      const searchkey = travelcoveragePlan==="worldwideexceptusa&canada" ? "worldwide_except_usa" : travelcoveragePlan;
      const plan = languageData?.travel_coverage_plan?.find(plan => plan.key === searchkey);
      setPackageData(plan);
    }
  }, [languageData?.travel_coverage_plan, travelcoverage,travelcoveragePlan]);
            ...
            <Col lg={6} className="right-side-bg-column">
              <div className="quote-content">
                ...
                {Array.isArray(packageData?.details) && packageData?.details.length > 3 && (
                  <div className="pop-link" onClick={handlePackage}>
