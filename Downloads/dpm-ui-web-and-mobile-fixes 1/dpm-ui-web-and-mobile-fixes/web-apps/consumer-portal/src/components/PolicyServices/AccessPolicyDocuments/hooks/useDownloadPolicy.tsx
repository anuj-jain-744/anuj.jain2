  ...
  const doApiCall = useCallback(async () => {
    ...
    try {
      ...
      if (!Array.isArray(base64StringArray)) {
        throw new Error(languageData?.response_data_is_not_an_array);
      }
      ...
      if (validPdfDocuments.length > 0) {
        ...
        console.warn(languageData?.no_valid_pdf_documents_found);
      }
    } catch (error) {
      console.error(languageData?.error_during_api_call, error);
    } finally {
      setIsLoading(false);
    }
  }, [isLoading]);
