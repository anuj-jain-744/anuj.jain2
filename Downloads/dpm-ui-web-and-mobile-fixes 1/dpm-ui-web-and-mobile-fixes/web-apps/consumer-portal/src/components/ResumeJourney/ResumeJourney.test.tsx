  ...
  beforeEach(() => {
    (useQuoteAndBuyContext as jest.Mock).mockReturnValue({
      ...
      setDriverDetailsResponseData: jest.fn(()=>{
        return [{ ownerId: '1', ownerDOB: '1990-01-01' },{ ownerId: '1', ownerDOB: '1990-01-01' }]
      }),
    });
...
describe("ResumeJourney useEffect block", () => {
  const mockSetDriverDetailsResponseData = jest.fn();
  const mockHandleDriverAdded = jest.fn();
  const mocksetCountryData = jest.fn();
  const mockJourneyData = JSON.stringify({
    vehicleDetails: { sequenceNumber: '123', customCardNumber: '456' },
    currentStep: '2',
    driverDetails: [{ ownerId: '1', ownerDOB: '1990-01-01' }],
    coverageType: 'comprehensive',
  ...
  const mockDriverDetailsResponse = {
    driverID: "1234",
    relationship: "",
  };
  ...
  const setup = (props = {}) => {
    (useQuoteAndBuyContext as jest.Mock ).mockReturnValue({
      ...
      handleDriverAdded: mockHandleDriverAdded,
      setCountryData:mocksetCountryData,
    });

    useApiCall.mockReturnValue({
      makeApiCall: jest.fn(),
      isLoading: false,
      data: mockDriverDetailsResponse, 
    });
    ...
    render(
      <ResumeJourney
        ...
        setLeftStep={jest.fn()}
        languageData={{
          resume_your_motor_insuranc: "Resume your motor insurance",
          dear_user_would_you_like: "Dear user, would you like to continue?",
          continue_from_where_left: "Continue from where you left",
          start_a_new_quotation: "Start a new quotation",
        }}
        {...props}
      />
    );
  ...
  it("updates driverDetailsResponseData and calls handleDriverAdded", async () => {
    setup();

    await waitFor(() => {
      // Check if setDriverDetailsResponseData is called
      expect(mockSetDriverDetailsResponseData).toHaveBeenCalledWith(
        expect.any(Function) // Because it receives a function callback
      );

      // Check if handleDriverAdded is called with driverDetailsResponse
      expect(mockHandleDriverAdded).toHaveBeenCalledWith(mockDriverDetailsResponse);
    });

    // Now simulate the internal logic of setDriverDetailsResponseData
    const updateDriverDetailsFn = mockSetDriverDetailsResponseData.mock.calls[0][0]; 
    const updatedDrivers = updateDriverDetailsFn([
      { driverID: "5678", relationship: "spouse" },
    ]); // Pass some initial driver data

    // Verify if driverDetailsResponse was added to the list
    expect(updatedDrivers).toEqual([
      { driverID: "5678", relationship: "spouse" },
      { driverID: "1234", relationship: "self" }, 
    ]);
  });
});
