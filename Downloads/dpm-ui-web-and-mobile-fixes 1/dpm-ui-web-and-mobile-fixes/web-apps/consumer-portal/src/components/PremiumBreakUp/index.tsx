...
import { JAVA_API_ROUTES, TRAVEL_COVERAGE_TYPE } from "../../constant";
...
const PremiumBreakUp: React.FC<IPremiumBreakUp> = ({
  ...
  useEffect(() => {
    if (!isHome) {
      const benefitsTotal = selectedBenefits.reduce(
        (acc, benefit) => acc + (benefit.price ?? 0),
        0
      );
  ...
  const renderHomePrice = () => {
    ...
    const price: { 'minFinalPrice': number, 'vatPrice': number, 'vatAmount': number, 'netPremium': number, 'adminFees': number } = calculatePremium({ [coverageType]: homePremiumResponse[coverageType] });
    return price;
  }
  ...
  useEffect(() => {
    if (isHome) {
      ...
      setSubtotal(price.minFinalPrice + price.adminFees);
      setVat(price.vatAmount);
      setPremium(price.netPremium);
      setAdminFees(price.adminFees)
    }
  }, [
    isHome,
    homePremiumResponse
      ...
      producttype === "Travel" ? (<React.Fragment>
        <div className="order-travel-body-content walaa-medium-500">
          <div className="order-travel-body-package-content">
            ...
            <div>{languageData?.sar} {getAmountText(renderPriceTravel()?.price)}</div>
          </div>
        </div>
        <div className="order-travel-body-content">
          <div className="travel-person">
            <div className="travel-person-header">
              ...
              <div className="travel-person-fee">{languageData?.sar} {getAmountText(renderPriceTravel()?.adminFee)}</div>
            </div>
          ...
          <div className="order-travel-body-package-content-light">
            ...
            <div>{languageData?.sar} {getAmountText(renderPriceTravel()?.adminFee)}</div>
          </div>
        </div>
        ...
        <div className="order-travel-body-content walaa-medium-500">
          <div className="order-travel-body-package-content">
            ...
            <div>{languageData?.sar} {getAmountText(travelsubtotal)}</div>
          </div>
        </div>
        <hr className="horizontal-travel-line" />
        <div className="order-travel-body-content">
          <div className="order-travel-body-package-content-light">
            ...
            <div>{languageData?.sar} {getAmountText(renderPriceTravel()?.vat)}</div>
          </div>
        </div>
        ...
        <React.Fragment>
          <div className="order-body-content">
            {getPremiumArr()?.sort((a, b) => Number(a.type) - Number(b.type))?.map((val: PremiumBreakdown) => {
              return (
                <div className="order-body-package-content" key={val?.amount}>
                  ...
                  <div>
                    {languageData?.sar} {val.sign === -1 ? "-" : ""} {getAmountText(val.amount)}
                  </div>
                </div>
              )
            })}
            <div className="order-body-package-content content-title">
              ...
              {repairTypeSelected === null ? (
                <div>
                  {languageData?.sar} {!isHome && getAmountText(renderPrice())} {isHome && getAmountText(renderHomePrice().minFinalPrice)}
                </div>)
                : (
                  <div></div>
                )
            ...
            {repairTypeSelected ? (
              <div className="order-body-package-content content-light">
                ...
                {repairTypeSelected !== null && (
                  <div>
                    {languageData?.sar} {!isHome && renderPrice()?.toFixed(2)}{" "}
                    {isHome && renderHomePrice().minFinalPrice.toFixed(2)}
                  </div>
                )}
              </div>
            ) : null}
            {selectedBenefits?.length > 0 && languageData?.additional_benefits && <div className="additional">{languageData?.additional_benefits}</div>}
            {selectedBenefits?.filter((value) => value.title).map((benefit, index) => (
          ...
          <div className="order-body-content">
            <div className="order-body-package-content content-subtotal">
              <div>{languageData?.subtotal}</div>
              <div>
                {languageData?.sar} {getAmountText(subtotal)}
              </div>
            </div>
          </div>
          <hr className="horizontal-line" />
          <div className="order-body-content">
            <div className="order-body-package-content content-light">
              ...
              <div>
                {languageData?.sar} {getAmountText(vat)}
              </div>
            </div>
          </div>
        </React.Fragment>)
  ...
  return (
    <Card className="order-summary-premium-breakup w-100">
      ...
      <div className="order-summary-footer walaa-medium-500">
        ...
        <div className="right">
          {languageData?.sar} {producttype === "Travel" ? getAmountText(renderPriceTravel()?.netpremium) : getAmountText(premium)}
        </div>
      </div>
      {data?.config && (
        <>
