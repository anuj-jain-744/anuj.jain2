  ...
  if (field === "policyCoverage") {
    ...
    setTravelers((prevTravelers) => {
      ...
      updatedTravelers[index] = {
        ...updatedTravelers[index],
        [field]: test.filter((item, index, current) => index === current.findIndex(t => t.coverageCode === item.coverageCode)),
      };
      ...
      >
          ...
          <Accordion.Header>
            <div className="acc-head">
              <div className="acc-head-strip"></div>
                  ...
                  <div className="acc-head-titles-align">{data?.adult_title} {i + 1} &nbsp;
                      <div className="acc-head-strip-sub">Traveller {i + 1}</div>
              </div>
              {validation === 1 ? (
        ...
        <Col>
          ...
          <FullCalender
            ...
            setValue={(value:Date | null) => handleInputChange(i, "adultPassportExp", value)}
            format={"DD/MM/YYYY"}
            isOn={isOn}
            showSwitch={false}
            setIsOn={setIsOn}
      ...
      <Row className="mb-3 form-block-2">
        <Col xs={6} md={4}>
          ...
          <FullCalender
            ...
            setValue={(value:Date | null) => handleInputChange(i, "adultDob", value)}
            format={"DD/MM/YYYY"}
            isOn={isOn}
            showSwitch={false}
            setIsOn={setIsOn}
        ...
        <Col xs={6} md={4}>
          <div className="form-element">
            ...
            <Form.Select onChange={(e)=>handleInputChange(i, "adultRelation", e.target.value)} aria-label="Default select realtion-select" size="lg">
              <option>Select</option>
              {data?.relations && Object.keys(data?.relations).map((relation: string) => (
                <option 
                selected={travelers[i]?.adultRelation === relation ? true : false} 
                value={relation}>{data?.relations[relation]}</option>
              ))  
              }
               ...
               <div className="benfit-content">
                 ...
                 >
                   ...
                   <div className="benfit-right">
                     <div
                       className={` ${
                        travelers[i]?.policyCoverage && checkStringAvailability("WSC", travelers[i]?.policyCoverage) ? "benfit-remove-btn" : "benfit-button"
                       }`}
                     >
                      {checkStringAvailability("WSC", travelers[i]?.policyCoverage) ? (
                      <div
                      onClick={() => handleRemoveCoverage(i, "WSC")}
                      className="benfit-button-text"
                    >
                      {` ${checkStringAvailability("WSC", travelers[i]?.policyCoverage) ? data?.remove_button : data?.add_button}`}
                    </div>
                    ) : (
                      <div
                       onClick={() => handleInputChange(i, "policyCoverage", {coverageCode: "WSC"})}
                       className="benfit-button-text"
                     >
                       {` ${!checkStringAvailability("WSC", travelers[i]?.policyCoverage) ? data?.add_button : data?.remove_button}`}
                     </div>
                    )}
                       {/* <div
                         onClick={() =>
                 ...
                 >
                   ...
                   <div className="benfit-right">
                     <div
                       className={` ${
                        travelers[i]?.policyCoverage && checkStringAvailability("CV", travelers[i]?.policyCoverage) ? "benfit-remove-btn" : "benfit-button"
                       }`}
                     >
                      {checkStringAvailability("CV", travelers[i]?.policyCoverage) ? (
                        <div
                        onClick={() => handleRemoveCoverage(i, "CV")}
                        className="benfit-button-text"
                      >
                        {` ${checkStringAvailability("CV", travelers[i]?.policyCoverage) ? data?.remove_button : data?.add_button}`}
                      </div>
                      ) : (
                        <div
                         onClick={() => handleInputChange(i, "policyCoverage", {coverageCode: "CV"})}
                         className="benfit-button-text"
                       >
                         {` ${!checkStringAvailability("CV", travelers[i]?.policyCoverage) ? data?.add_button : data?.remove_button}`}
                       </div>
                      )}
                     </div>
                   </div>
