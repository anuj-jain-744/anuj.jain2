...
import { capitalizeNameFirstLetter, sanitizeHtml } from "@dpm/shared-module";
...
const DeclarationCard: React.FC<DeclarationCardProps> = ({
  ...
  const onChangeHandler = (key: string, value: string, mandatoryValue: string = '') => {
    if (mandatoryValue && mandatoryValue === declareOptions[value]) {
      setShowError(true)
    }
  ...
  function mapDeclarationItems(items: DeclarationProps[], starts: number) {
    return items?.map((declareItem, idx: number) => {
      ...
      return <>
        <div key={idx} className="declare-container">
          ...
          <div className="declare-toggle">
            <ToggleButton
              ...
              isActive={active}
              onChange={() => onChangeHandler(declareItem?.key, active, declareItem?.value)}
            />
          </div>
        </div>
        {declareItem?.key === coastalLine && active && <div className="coastal-line">
          <CustomAlertBox variant={warningLabel}>
            <div
              dangerouslySetInnerHTML={{
                __html: sanitizeHtml(languageData?.propertyNearCoastlineWarning),
              }}
            />
          </CustomAlertBox>
        </div>}
  ...
  return (
    <div className="card-section-declare background-color-white">
      <AlertBox
        title={''}
        description={languageData?.declartion_error_message}
        showAlertModal={showError}
      ...
      <div className="declare-wrap">
        {languageData?.welcome_message && <div className="welcome-message">{ownerFullNameEnglishCapitalized}, {languageData?.welcome_message}</div>}
