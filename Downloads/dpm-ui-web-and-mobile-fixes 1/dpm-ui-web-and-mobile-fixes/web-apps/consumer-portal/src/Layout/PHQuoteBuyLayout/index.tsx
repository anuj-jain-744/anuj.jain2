...
import SummaryTabSwitch from "components/SummaryTabSwitch";
import CommonOffCanvas from "components/CommonOffCanvas";
...
import { apiRoutes, PREMIUM_DATA, commonKeywords, HOME_COVERAGE_PLANS_TYPES, JAVA_API_ROUTES } from "constant";
...
export const PHQuoteBuyLayout: React.FC<LayoutProps> = ({ leftPanel, rightPanel, languageData, navigateTo, prodID }) => {
  ...
  const isSmallScreen = initialWidth.current <= 1200;
  ...
  const setPolicyRenewalChanges = async (policyData) => {
    ...
    setHomePolicyRenewal({
      ...
      coverageType: policyForm?.planCode,
