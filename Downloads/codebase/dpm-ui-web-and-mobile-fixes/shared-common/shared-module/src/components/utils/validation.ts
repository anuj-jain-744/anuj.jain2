...
export const iqmaIdNationalIdValidation = (input: string) => {
  if (input === "") {
    return false;
...
export const iqmaIdNationalIdValidationOnBlur = (input: string) => {
  let flag = false;
...
export const refNoValidation = (input: string) => {
  if (input && input.length > 0) {
    if (/^[A-Za-z]/.test(input) && !/^[A-Za-z]{0,2}[0-9]{0,10}$/.test(input)) {
      //## Najm case ##
    ...
    if (
      ...
      (!/\d$/.test(input) ||
        input.length > 10 ||
        (input.length === 10 && !/^[4567][0-9]{0,10}$/.test(input)))
    ) {
      //## Police case ##
    ...
    if (/^[123890]/.test(input) && (!/\d$/.test(input) || input.length > 30)) {
      //## Other case ##
...
export const claimIdCaseNoValidation = (
  ...
  if (claimIdinput && ownerIdInput === "") {
    ...
    if (
      ...
      (ownerIdInput.length === 10 && /^[127]\d{0,9}$/.test(ownerIdInput))
    ) {
      flag = true;
    }
  }
...
export function isValidAlphanumericKey(key: string) {
...
export const claimIdValidationOnBlur = (input: string) => {
  ...
  if (input === "") {
    ...
    if (
      !(
        (input.length === 19 &&
          /^C-([A-Z]\d{2}|[A-Z]{2}\d)-\d{2}-\d{3}-\d{6}$/.test(input)) ||
        (input.length === 21 &&
          /^C-([A-Z]\d{2}|[A-Z]{2}\d)-\d{2}-\d{5}-\d{6}$/.test(input))
      )
    ) {
      flag = true;
    }
...
export const validateIbanNonSA = (
  inputString: string,
  maxLength?: number,
  minLength?: number
) => {
  // Check if the input is not less than 16 or not more than 34 characters
  if ((minLength && inputString.length < minLength) || (maxLength && inputString.length > maxLength)) {
    return false;
  }

  // Check if the first 2 characters are alphabets
  if (!/^[A-Z]{2}[0-9][A-Z0-9]*$/.test(inputString)) {
    return false;
  }
  return true;
};
...
export const isValidPassword = (password: string): boolean => {
  const passwordRegex =
    /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[^A-Za-z0-9]).{10,50}$/;
  return passwordRegex.test(password);
};

export const isValidEstimateAmount = (val: string) => {
  // Allow empty string for controlled input
  if (val === "") return true;
  // Only allow digits and a single dot
  const valid = /^[1-9][0-9]*\.?[0-9]*$/.test(val);
  return valid;
};

export const isInvalidKey = (key: string): boolean => {
