# Walaa Corporate Website
 
This module serves for Walaa corporate website components. Using this we can provide details for products and services which is offered by Walaa Insurance. It leverages React, TypeScript, and Vite to provide a fast and efficient development experience with Hot Module Replacement (HMR).
Contents for this website is fetched from a separate CMS website so that Client can change the content as and when it is required.
...
When creating a new module within the `@dpm/corporate-portal`, it's crucial to ensure both functionality and robustness through comprehensive testing. This includes writing both positive and negative test cases. Here's a guide to get you started:
...
4.  **Test the Component**: To test all the the component, for debugging and run fast we can passed option as  --coverage=false run: 

```bash
yarn test 
yarn test `fileName`  --coverage=false  --maxWorkers=4
