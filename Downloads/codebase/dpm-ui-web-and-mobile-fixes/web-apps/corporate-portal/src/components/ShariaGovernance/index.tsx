              ...
              <div className="related-links">
                {links.map((link, index) => (
                  <React.Fragment key={index}>
                    <div className="link" onClick={() => navigateTo && navigateTo(link.route)} role="button" tabIndex={0} onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                            e.preventDefault();
                            navigateTo && navigateTo(link.route)
                        }
                    }}>
