...
import {  productIDs, INVALID_OTP_ERROR_CODE, OTP_TIMER, RESEND_OTP_TIMER } from "../../constant";
...
export const ClaimWidget: React.FC<ClaimWidgetProps> = ({ formHeaderData, navigateTo }) => {
  ...
  const [sessionId, setSessionID] = useState<string>(""); // session ID for OTP generation
  const propsData = location?.state?.data || {};
  const {lineOfBusiness, claimNo} = propsData;

  useEffect(() => {
    if (lineOfBusiness && claimNo && isAuthenticated && userId) {
      generateClaimTrackOtp();
    }
  }, [lineOfBusiness, claimNo, userId, isAuthenticated]);
  ...
  const checkIsButtonDisabled = () => {
    ...
    const areMandatoryFieldsFilled = mandatoryFieldValues.every(
      (val) => val !== null && val !== undefined && val !== ""
    );

    const isNationalIdValid =
      formInput.national_id_iqama_id_corp_id !== undefined &&
      formInput.national_id_iqama_id_corp_id.length === 10 &&
      /^\d{10}$/.test(formInput.national_id_iqama_id_corp_id);

    return !(areMandatoryFieldsFilled && isNationalIdValid);
  };
  ...
  const generateClaimTrackOtp = async (isResend: boolean = false, sessionId?: string) => {

    const sessionSecretID = isResend && sessionId ? sessionId : getRandomString(16); // setting random session ID
    setSessionID(sessionSecretID); // set session ID for OTP generation
    // setSessionID(sessionSecretID); // once sessionSecretID will available make it dynamic

    const prodLOB = productIDs.motor.charAt(0).toUpperCase() + productIDs.motor.slice(1);
    // product Motor no need to pass prodLOB from UI Java will handle the rest
    ...
    const urlRequestBody: GenerateOTPPayload = {
      ...
      const requestBody: GenerateOTPPayload = {
        ...
        claimNoOrCaseRefNo: claimNo ? claimNo : formInput.case_ref_no_claim_id,
  ...
  useEffect(() => {
    if (responseData) {
      ...
      if (!isAuthenticated) {
        ...
        navigateTo?.(`/Claim/Track-Your-Claim`, {
  ...
  useEffect(() => {
    if (validateData) {
      setShowOTPModal(false);
      navigateTo?.(`/Claim/Track-Your-Claim`, { trackClaimData: validateData });
      // setting data here to pass to next page
  ...
  const handleResend = (sessionId: string) => {
    generateClaimTrackOtp(true, sessionId);
  }
  ...
  return (
    <>
      ...
      <OTPValidation
        ...
        timerResend={OTP_TIMER}
        resendOtpTimer={RESEND_OTP_TIMER}
          ...
          <div className="claim-body">
            <Row className="claim-content">
              ...
              <Col className="claim-text col-md-10 col-lg-10 col">
                ...
                <Form className="form-claim-track" data-testid="claim-form" onSubmit={handleSubmit} >
                  {formFields.map((val) => (
                    <Form.Group controlId="formBasic" key={val.field_name} className="form-group-fields">
                      ...
                      {val.field_type === "webform_actions" && (
                        <span className={`icon ${checkIsButtonDisabled() ? "disabled" : ""}`}>
