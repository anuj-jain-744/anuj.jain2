...
export const SetPassword: React.FC<SetPasswordProps> = ({
  ...
  const validatePasswordMatch = () =>{ 
    const newPasswordValue = formState.new_pasd?.value || "";
    const confirmPasswordValue = formState.confirm_pasd?.value || "";
    if (newPasswordValue && confirmPasswordValue && newPasswordValue !== confirmPasswordValue) {
      setApiError({
        'confirmPassword': (formElementsData as { data: { passwords_do_not_match: string } })?.data?.passwords_do_not_match
      });
      return false;
    }
    return true;
  ...
  },[formState.new_pasd?.value, formState.confirm_pasd?.value]);
  ...
  useEffect(() => {
    if (errors || signupError) {
      const err = errors ?? signupError;
      setApiError({
        'confirmPassword': err?.messages?.message_en
      })
    } else if (data || signupData) {
      const successMessage = LoginContext.signUpForm ? (formElementsData as { data: { signup_success_msg: string } })?.data?.signup_success_msg : (formElementsData as { data: { forgotpassword_success_msg: string } })?.data?.forgotpassword_success_msg;
          ...
          <Form.Group>
            <div className="user-input">
              <FormsField
                ...
                onFieldBlur={(fieldName, value, isValid) => handleBlur(fieldName, value as string | number, isValid)}
