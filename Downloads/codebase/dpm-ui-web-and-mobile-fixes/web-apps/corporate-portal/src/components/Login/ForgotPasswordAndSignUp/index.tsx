...
export const ForgotPasswordAndSignUp:React.FC<ForgotPasswordAndSignUpProps>  = ({
    ...
    const url = LoginContext?.signUpForm ? "/UserSignup/GenerateOtp" : "/UserLogin/CheckUserExist";
    const apiType = LoginContext?.signUpForm ? 14 : 13;
    const { makeApiCall, isLoading, errors, data } = useApiCall(apiType, url, "post");
    const fieldstoexclude = ["mobile_no",]; // fields to exclude for forgot password form
    ...
    const handleSubmit = () => {
      if(allFieldsValid()){
        ...
        if(LoginContext?.signUpForm) {
          ...
          LoginContext.setnavigateToErrorFrom(6);
        }        
        makeApiCall(payload);
      }
    }
    ...
    useEffect(()=>{
        ...
        const filteredArray = LoginContext?.signUpForm ? formData?.fields : formData?.fields?.filter((item: { field_name: string }) => !fieldstoexclude.includes(item.field_name));
    ...
    useEffect(() => {
      if(errors) {
        if(errors?.code === loginModuleErrorCode.DTXJLOG5006){
          setCurrentStepValue(0);
          LoginContext?.setWarningMessage(errors?.messages?.message_en);
        }else{          
          LoginContext?.signUpForm ? 
          setApiError({'mobileNumber' : errors?.messages?.message_en}) :
          setApiError({'nationalID' : errors?.messages?.message_en});
        }
      } else if(data) {
        LoginContext?.signUpForm ? setCurrentStepValue(1) : setCurrentStepValue(6);
    ...
    <React.Fragment>
        ...
        <div className="form-head">
            ...
            <div className="signup-title">
              {formElementsData?.data?.welcome_message}
            </div>
            }
        </div>
