...
const ServicesTab = ({
  ...
  useEffect(() => {
    ...
    if (window.innerWidth > BreakPoints.Tablet_Max && currentTabItems.length > responsive.superLargeDesktop.items){
      setIsMobile(true);
    }
    ...
    <>
              ...
              {Object.keys(servicesData).map((checkbox, index) => (
                <div
                  ...
                  role="button" tabIndex={0} onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                        e.preventDefault();
                        handleProductToggleClick(index)
                    }
                }}
                >
                  <span>{checkbox}</span>
      ...
      {Object.keys(servicesData).length > 0 ? (
        Object.keys(currentTabData).length > 0 ? (
          ...
          <div className={`${hideProductTab ? 'border-top-left' : ''} service-white-container ${isMobile && 'service-tab-dots'}`}>
            {/* Sub-tabs */}
            ...
            <animated.div style={isVisible ? productData : {}} >
              <Row className={`align-items-start tab-content ${os}`}>
