...
export const Support: React.FC<SupportProps> = ({
  ...
  return (
    <div className={`support_section ${typeClass ? typeClass : ""}`}>
  ...
  <Row className="rtl-card career-cards-container">
    {supportData.map((data, id) => (
      <Col key={id} xs={12} sm={12} md={6} lg={6}>
        <div
          ...
          role="link"
          tabIndex={0}
          onClick={() => data.link && navigateTo(data.link)}
          onKeyDown={(e) => {
            if (e.key === "Enter" || e.key === " ") {
              e.preventDefault();
              data.link && navigateTo(data.link);
            }
          }}
        ...
        >
          ...
          <div className="support_contact walaa-medium-500">
            {data.contact && (
              ...
              >
                {data.tinyicon && <img src={data.tinyicon} alt="support" />}
                {data.contact}
              </a>
            )}
          </div>
