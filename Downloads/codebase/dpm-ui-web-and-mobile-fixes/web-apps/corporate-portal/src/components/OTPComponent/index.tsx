...
const OTPComponent: React.FC<Props> = (props) => {
  ...
  const [timeLeft, setTimeLeft] = useState(
    props?.timerResend ? props.timerResend : 120
  ); // timeOTP seconds
  ...
  const getFormattedOTPExpiryMessage = (timeLeft: number): JSX.Element => {
    ...
    const msg = props?.languageData?.your_otp_will_expire
      ? props?.languageData?.your_otp_will_expire
      : "";
          ...
          <Modal.Header closeButton>
            <Modal.Title>
              <h4 className="modal-title">
                {props?.languageData?.confirm_otp}
              </h4>
            </Modal.Title>
          </Modal.Header>
          <Modal.Body>
            <div
              ...
              dangerouslySetInnerHTML={{
                __html: props?.languageData?.otp_info_message,
              }}
            ></div>
            <div className="frame-3">
              <span className="text-wrapper-3">
                {props.languageData?.enter_otp_code}
              </span>
            ...
            <div className="expireOTPSection">
              ...
              <div className="expireOTPInnerDiv2">
                {timeLeft == 0 && !isDisabled ? (
                  <a className="hyperlink" target="#" onClick={handleResendOTP}>
                    {" "}
                    {props?.languageData?.resend_otp}{" "}
                  </a>
                ) : (
