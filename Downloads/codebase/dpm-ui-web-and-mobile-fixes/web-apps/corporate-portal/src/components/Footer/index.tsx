...
import { FooterCruve, ScrollUpIcon } from "../../assets/Footer";
...
export const Footer: React.FC<FooterProps> = ({
  ...
  const [footerCollapsed, setFooterCollapsed] = useState<boolean>(true);
  ...
  const footerLink = chunkArray(Object.keys(footerMenus || {}), 4).map(
    (val: string[]) => {
      return val.map((subMenu: string) => {
        return {
          label: subMenu,
          subLabel: footerMenus[subMenu],
        };
      });
    }
  );
 ...
 const renderSubMenuLinks = (
  item: { label: string; subLabel: { menuUrl: string; linkName: string }[] },
  navigateTo?: (url: string) => void
) => {
  if (!item?.subLabel || !Array.isArray(item.subLabel)) return null;

  return item?.subLabel.map((subItem, subIndex) => (
    <a onClick={() => navigateTo && navigateTo(subItem?.menuUrl)} key={subIndex} role="button" tabIndex={0} onKeyDown={(e) => {
      if (e.key === 'Enter') {
        e.preventDefault();
        navigateTo && navigateTo(subItem?.menuUrl);
      }
    }}>
      <li className="list-content walaa-regular-400" key={subIndex}>
        {subItem?.linkName}
      </li>
    </a>
  ));
  ...
  return (
    <section className="footer-section">
      <div className="footer-curve-wrapper">
        <img src={FooterCruve} alt="Footer curve" className="footer-curve" />
        <div className="footer-scroll-up-container">
          <img
            src={ScrollUpIcon}
            alt="scroll-up"
      ...
      <Container fluid className="footer-content-next">
        <div className="accordian-wrapper">
          <ExpandCircleDownOutlinedIcon
            className={`${!footerCollapsed ? "icon-up" : ""} accordian-down`}
        ...
        <Row className="footer-content-wrapper">
          <Col lg={3} md={6} sm={12} className="icon-wrapper">
            <ul className="list-unstyled">
              <li>
                <div
                  className={`logo-container walaa-regular-400 ${!footerCollapsed ? "footer-collapsed" : ""
                    }`}
          ...
          <Col lg={9} md={6} sm={12} className="footer-content-row">
                      ...
                      <Col key={itemIndex} lg={3} md={6} sm={12}>
                        <div className="footer-content-list">
                          <ul className="list-unstyled">
                            ...
                            {expandedItems[item.label] && renderSubMenuLinks(item, navigateTo)}
                          </ul>
                        </div>
                      </Col>
                    )
            ...
            <Row className="social-wrapper">
              ...
              <Col lg={4} md={12} sm={12}>
                <h3 className="social-title walaa-medium-500">{socialHandles?.title}</h3>
              ...
              {privacy?.data?.length > 0 &&
                privacy.data.map((item, index) => (
                  <div
                    ...
                    key={index}
                    onClick={() => navigateTo && navigateTo(item?.menuUrl)} //To Do : add test
                    role="button" tabIndex={0} onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                  ...
                  >
                    <span onClick={handleScrollToTop} role="button" tabIndex={0} onKeyDown={(e) => {
                      if (e.key === 'Enter') {
                        e.preventDefault();
                        handleScrollToTop();
                      }
