...
export const RenderContentNavigateTo: React.FC<
  RenderContentNavigateToProps
> = ({
  cIdx,
  menuUrl,
  ...
  return (
      ...
      onKeyDown={(e) => {
        if (e.key === "Enter") {
          ...
          if (!isUserAuthenticated) {
            ...
            navigate(menuUrl, { state: { isProducts: true } });
          }
          handleDropdownEvent("", false, 0);
        }
      }}
    >
      ...
      <span className="sub-menu-text">{linkName}</span>
      {!IconsSet[attributes?.class[0]] && <ArrowForwardIcon />}
    </div>
  );
};
        ...
        return (
          <React.Fragment>
            <Col
              ...
              className="container-width product-content-container"
            >
              <div
                className={`${
                  isUserAuthenticated ? "toggle-wrapper" : ""
                } product-toggle-wrapper-Ui-Tabs`}
              >
                <div className="product-toggle walaa-medium-500">
                  {childrens?.map((productOption, index) => (
                    <HiddenButtonWrapper
                      key={"product-dropdown-Option_" + index}
                      onClick={() =>
                ...
                {activeProduct &&
                  childrens[activeProduct?.index]?.childrens?.map(
                    ({ linkName, attributes, menuUrl }, cIdx) => (
                      <Col
                        md={activeProduct.label !== "SME" ? "6" : "12"}
                        key={cIdx}
                        className="product-wrapper-col"
                      >
                        <RenderContentNavigateTo
                          ...
                          linkName={linkName}
                          menuUrl={menuUrl}
                          handleDropdownEvent={handleOnMouse}
            ...
            {childrens[activeProduct?.index]?.menuimage &&
              childrens[activeProduct?.index]?.menuimage?.image_urls && (
                <Col
                  md={4}
                  className="container-padding product-banner-wrapper"
                >
                  <div className="product-banner-content">
                    ...
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: sanitizeHtml(
                          childrens[activeProduct?.index]?.link_content || ""
                        ),
                      }}
                    />
                  </div>
        ...
        return (
          <React.Fragment>
            {media_type_val && !video_url && (
              <React.Fragment>
                <Col md={7} className="app-content-col">
                  <Row className="content-direction">
                    {childrens.map(
                      ({ linkName, attributes, menuUrl }, cIdx) => (
                        <Col
                          lg={3}
                ...
                <Col md={5} className="app-downloads-wrapper">
                  <div className="app-download-content">
                    <div
                      dangerouslySetInnerHTML={{
                        __html: sanitizeHtml(link_content),
                      }}
            ...
            {!video_url && !media_type_val && (
              <Col md={6} xl={10} className="app-content-col">
                <Row>
                  {childrens.map(
                    ({ linkName, attributes, menuUrl, link_content }, cIdx) => (
                      <Col
                        lg={3}
                        md={4}
                        sm={6}
                  ...
                  <Row className="video-banner-row">
                    {childrens.map(({ linkName, menuUrl }, cIdx) => (
                      <Col md={12} key={cIdx} className="video-banner-col">
                        ...
                        >
                          <div className="video-banner-content walaa-medium-500">
                            <span>{linkName}</span>
                            <ArrowForwardIcon />
                          </div>
                        </HiddenButtonWrapper>
