...
import { cmsAPIRoute, productType } from "@src/constants";
import "./index.scss";

export default function Success() {
  const transactionId = useParams();
  const { productCode, endrosmentNo } = transactionId?.transactionid ? processTransactionId(transactionId?.transactionid): { productCode: "", endrosmentNo: null };
  ...
  const breadcrumbData = [
    { label: "Home", route: "/" },
    { label: "Product", route: state?.paymentConfig?.config?.productCodeTitle[productCode] === productType.motor ? "/personal/product/motor" : "/" },
    { label:  state?.paymentConfig?.config?.productCodeTitle?.[productCode], route: "/" },
  ];

  const breadcrumbEndo = [
  ...
  return (
    <div>
      ...
      <HighlighterBanner
        showInput={false}
        title={pageTitle ?? ""}
        breadcrumbsData={endrosmentNo ? breadcrumbEndo : breadcrumbData}
