...
const PersonalProductLanding: React.FC = () => {
  ...
  const breadcrumbData = generateBreadcrumbs(
    [
      ...
      {
        label:
          // state?.productData?.data?.breadcrumb
          trimmedStr, //To be replaced post API Fix of white space
        route: "/section",
      },
    ],
    currentLanguage
  ...
  //TODO: This needs to be handled from Common function, as this is common for all the pages
  ...
  const fetchAllData = useCallback(async () => {
    const [
      ...
      fetchData(
        `${cmsAPIRoute["personalProduct"]}${product_name}`,
        currentLanguage
      ),
  ...
  const faqContent = state?.faqData?.product_faq?.[faqProductListing];
    ...
    <PublicLayout>
      <React.Fragment>
        ...
        {!loading && (
          <React.Fragment>
            {seoTags}
            <Template templateType="product" data={combinedData} isProducts={location?.state?.isProducts} />
          </React.Fragment>
        )}
      </React.Fragment>
    </PublicLayout>
...
export default PersonalProductLanding;
