...
const CoveragePlan: React.FC<ICoveragePlanProps> = ({ languageData }) => {
  ...
  const { viewPolicyCall,isLoadingCalculatePremium:isLoadingCalculatePremiumForViewPolicy,viewPolicyLoading } = useViewPolicyCall({
    setShowAlertModal,
    setApiErrorMessage,
  });
 ...
 // for Third party & Renew policy case only
  ...
  return (
    <div data-testid="CoveragePlan-test">
      {(homePremiumHandling.loading || isLoadingCalculatePremiumForViewPolicy || viewPolicyLoading) && <LoaderOverlay />}
      <AlertBox
