...
const DriverDetails: React.FC<DriverDetailsProps> = ({ languageData, handleClose}) => {
  const { driverDetails: driverDetailsData, driverDetailsResponseData, setDriverDetailsResponseData, setDriverDetails, setSelectedDriverID, selectedDriverID} =
  ...
  return (
    <div className={style.container}>
      {updatedDriverData?.map((driver, index) => (
        <div className={style.row} key={driver.driverID + index}>
          <div className={style.iconContainer}>
            <div className={style.icon}>
              <img
                src={getGenderProfileIcon(driver?.gender)}
                width={"40px"}
                height={"40px"}
                alt="Driver Imaga"
              />
          ...
          <img src={verticleLine} alt="verticleLine" />
          <div className={`${style.iconContainer} ${style.deleteIconStyle}`}>
            <div
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                  e.preventDefault();
                  handleDriverDetails(driver?.driverID);
                }
              }}
            >
              <div
      ...
      {showRemoveDriverModal && (
        <RemoveVehicleModal
          ...
          description={
            languageData?.are_you_sure_you_want_to_remove_driver ??
