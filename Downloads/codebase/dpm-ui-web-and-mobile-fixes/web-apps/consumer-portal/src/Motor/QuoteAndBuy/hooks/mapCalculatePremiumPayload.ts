...
export const DEFAULT_SCHEME_CODE = "P1";
...
export const mapCalculatePremiumPayload = (
  ...
  const getCountryNameById = (codeId: string): string | null=> {
    const country = countryData?.find((country: Country) => parseInt(country?.codeId) === parseInt(codeId));
    return country ? country.codeDesc : null;
  }

  const getValueByInput = (input: unknown): string => {
    return input ? "1" : "0";
  }
  ...
  return {
    policyRisk: {
      ...
      fireExtinguisher: getValueByInput(vehicleData.additionFeatures?.commercialVehicle?.fireExtinguisher),
      antiLockBrakingSystem:getValueByInput(vehicleData.additionFeatures?.safety?.antiLockBrakingSystem),
      vehicleAutoBraking: getValueByInput(vehicleData.additionFeatures?.safety?.automaticBrakingSystem),
      vehicleABS: getValueByInput(vehicleData.additionFeatures?.safety?.antiLockBrakingSystem),
      vehicleCruiseControl: getValueByInput(vehicleData.additionFeatures?.features?.cruiseControl),
      vehicleAdaptiveCruiseControl: getValueByInput(vehicleData.additionFeatures?.features?.adaptiveCruiseControl),
      vehicleRearSensors: getValueByInput(vehicleData.additionFeatures?.camera?.rearParkingSensor),
      vehicleFrontSensors: getValueByInput(vehicleData.additionFeatures?.camera?.frontSensor),
      vehicleRearCamera: getValueByInput(vehicleData.additionFeatures?.camera?.rearCamera),
      vehicleFrontCamera: getValueByInput(vehicleData.additionFeatures?.camera?.frontCamera),
      vehicle360Camera: getValueByInput(vehicleData.additionFeatures?.camera?.degreeCamera),
      ...
      drivers: driverData.map((driver, index) => ({
        ...
        licenseType: driver.licenseType ?? 1, // Default value as Suggested by Dikesh/Tejas
    ...
    policyBasic: {
      ...
      effectiveDate: policyStartDate ? formatDateToISO(policyStartDate) : new Date(Date.now() + 24 * 60 * 60 * 1000).toISOString().split('T')[0], // Default to tomorrow if not provided because effective date is one day after the quotation date
    }
  };
};
