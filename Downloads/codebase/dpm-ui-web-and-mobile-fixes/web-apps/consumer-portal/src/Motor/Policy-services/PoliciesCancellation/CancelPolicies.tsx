...
function CancelPolicy({ policyDataObj, navigateTo }: Readonly<CancelPolicyProps>) {
  ...
  const handleDropdownChange = (
    ...
    setIsFlagged(value !== languageData?.select);
  ...
  const repairType = data?.policyLob[0]?.policyRisk[0]?.repairCondition;
  ...
  const userIban  = policyData?.data?.iban
  ...
  const isSubmitDisabled =
    ...
    useEffect(() => {
      if (userIban && isDropdownOpen) {
        setIban(userIban);
        setDropdownOpen(false);
        if(!userIban.startsWith(SA)) {
          setIsValidNonSAIban(true);
        }
      }
    }, [userIban, isDropdownOpen]);
            ...
            <div className="right-card-policy">
              {selectedPolicyNumber && (
                <PolicyCard
                  ...
                  vehicleMakeModel={truncateName(capitalizeNameFirstLetter(`${vehicle?.make} ${vehicle?.model}`),12)}
                  repairType={repairType}
                />
              )}
                        ...
                        <div className="bottom-card">
                          ...
                          {drivers
                              ...
                              .map((driver: Driver, index: number) => (
                                <div className="driver" key={driver.driverName + index}>
