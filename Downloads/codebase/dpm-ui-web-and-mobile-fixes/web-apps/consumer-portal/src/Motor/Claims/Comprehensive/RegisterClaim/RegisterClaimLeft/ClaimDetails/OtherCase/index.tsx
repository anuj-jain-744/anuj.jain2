...
const OtherCase: React.FC<IOtherCase> = ({
  ...
  const [estAmountUpdateOthers, setEstAmountOthersUpdate] = useState<
    string | number | null
  >(null);
  // set estimate amount update
  useEffect(() => {
    (estAmountUpdate || estAmountUpdate?.toString()?.length === 0) &&
      setEstAmountOthersUpdate(estAmountUpdate);
  }, [estAmountUpdate || estAmountUpdate?.toString()?.length === 0]);
  //other case
  ...
  const changeSelectHandler = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    onChangehandler({
      target: { name: lossType, value: value },
        ...
        othersClaimInfo?.ClaimType === motorCoverageTypes?.comp ? (
          ...
          <React.Fragment>
            <div className="px-4 mx-2">
              ...
              {/* upload documents */}
              {matchDocList && matchDocList?.length > 0 && (
