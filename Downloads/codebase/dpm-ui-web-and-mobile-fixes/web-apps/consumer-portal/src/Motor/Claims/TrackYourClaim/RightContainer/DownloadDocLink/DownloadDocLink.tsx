...
const DownloadDocLink: React.FC<DownloadDocLinkProps> = ({ 
  ...
  const downloadParams: DownloadParams = useMemo(() => ({
    ...
    // Function to extract all docName from api
function extractDocName(): string[] {
  const documentNames: string[] = [];
  const claimTrackingDetails = trackNewData?.claimTrackingDetails;

  if (Array.isArray(claimTrackingDetails)) {
      claimTrackingDetails.forEach((detail) => {
          if (Array.isArray(detail.printDoc)) {
              detail.printDoc.forEach((doc) => {
                  if (doc.docName) {
                    documentNames.push(doc.docName);
                  }
              });
          }
      });
...
const claimDocuments = extractDocName();
  ...
  const {
    ...
    downloadParams,
    documentTypes: ['custom', claimDocuments] // custom doc download
  });
            ...
            {hasDocuments && (
              <>
                <div className={style.docPdfContainerBox}>
                  {claimDocuments.map((docName, index) => {
                    ...
                    return (
                      <div
                        ...
                        role="button" tabIndex={0} onKeyDown={(e) => {
                          if (e.key === 'Enter') {
                            e.preventDefault();
                            !isDownloading && handleDownload(docName);
                          }
                        }}
                      >
                ...
                {extractDocName().length > 1 && (
                  ...
                  role="button" tabIndex={0} onKeyDown={(e) => {
                    if (e.key === 'Enter') {
                      e.preventDefault();
                      !isDownloadDisabled && handleDownloadAll();
                    }
                  }}
                >
