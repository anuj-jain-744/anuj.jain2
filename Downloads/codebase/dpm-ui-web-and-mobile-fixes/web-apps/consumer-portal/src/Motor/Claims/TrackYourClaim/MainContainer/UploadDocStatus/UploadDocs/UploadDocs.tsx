...
const UploadDocs: React.FC<UploadDocsProps> = ({ setIsSuccess, isSuccess }) => {
  ...
  ? trackNewData.claimTrackingDetails
      .filter((claimTrackingDetail) => claimTrackingDetail.taskStatus === taskConstants.PENDING)
      .flatMap((claimTrackingDetail) =>
        Array.isArray(claimTrackingDetail.uploadDocumnets)
          ? claimTrackingDetail.uploadDocumnets
              .filter((doc) =>
                !doc.status?.toLowerCase().includes("received")
              ) // Exclude already uploaded ones
              .map((doc) => ({
                id: doc.documentId,
  ...
  const handleFileRemove = (id: string) => {
    ...
    if (targetItem) {
      ...
      toast.info(`${trackClaimInfo?.file_removed} for ${targetItem?.name}`, { autoClose: TOAST_AUTOCLOSE_TIMER || false });
    }
  };
  ...
  const convertToBase64 = (file: File): Promise<string> => {
    return new Promise((resolve, reject) => {
      ...
      reader.onerror = (error) => reject(new Error(`File reading error: ${JSON.stringify(error)}`));
    });
  };
  ...
  const updateItemsFile = (
    ...
    setNewFileItems((prevItems) =>
      prevItems.map((item) =>
        item.id === id ? { ...item, file,name:file?.name, base64,error: null } : item
      )
    );
  };
    ...
    <div className={style.tycContainer}>
      <Accordion
        className={`${isOpen ? "accor-open" : "accor-close"} accorUploadDocs`}
         defaultActiveKey={isSuccess ? "0" : undefined}
      >
