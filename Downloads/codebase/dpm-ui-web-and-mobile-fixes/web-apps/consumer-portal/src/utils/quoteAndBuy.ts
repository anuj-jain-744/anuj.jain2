...
import {
  ...
  endorseTypes,
  genderCodes,
  travelClaimBufferDurationDays,
} from "../constant";
import { covergaeTypes, familtyFlowConstants } from "components/Travel/constantsTravel";
...
export const getTotalDiscountApplied = (premiumData) => {
  return premiumData?.premiumBreakdowns.reduce((sum, item) => {
    return item.sign === -1 ? sum + item.amount : sum;
  }, 0) ?? 0;
};

export const getBasePremium = (policyData) => {
  return policyData.reduce((total, person) => {
    const ctiCoverage = person?.policyCoverage.find(
      coverage => coverage.coverageCode === covergaeTypes.comprehensive
    );
    return total + (ctiCoverage?.premiumInfo?.annualPremium ?? 0);
  }, 0) ?? 0;
};

export const getTravelDuration = (
  travelEndDate: DateObject | string,
  travelStartDate: DateObject | string
) => {
  const formattedTravelStartDate =
    typeof travelStartDate === "string"
      ? new DateObject(travelStartDate).format(dateFormats.date)
      : travelStartDate.format(dateFormats.date);
  const formattedTravelEndDate =
    typeof travelEndDate === "string"
      ? new DateObject(travelEndDate).format(dateFormats.date)
      : travelEndDate.format(dateFormats.date);
  return subtractDates(formattedTravelEndDate, formattedTravelStartDate) + 1;
};

export const getDiscountDetails = (
  discountData: Array<{ type: string; amount: number; sign: number }>,
  langData: { premium_breakdown?: Array<{ code: number; description: string }> }
): Array<{ id: string; description: string; amount: number }> => {
  if (Array.isArray(discountData)) {
    const discounts = discountData.filter((item) => item.sign === -1);
    const discountOptions = Array.isArray(langData?.premium_breakdown)
      ? langData.premium_breakdown
      : [];
    return discounts.map((item, index) => {
      const discountOption = discountOptions.find(
        (value) => value.code === parseInt(item.type)
      );
      return {
        id: index.toString(),
        description: discountOption ? discountOption.description : "",
        amount: -item.amount,
      };
    });
  }
...
export const hasTravelBenefit = (
  person: Travel.Traveler,
  benefitCode: string
): boolean => {
  return (
    person.policyCoverage?.some((item) => item.coverageCode === benefitCode) ??
    false
  );
};
...
export const isTravelClaimDisabled = (policy: PolicyDetail): boolean => {
  let result = false;
  if (policy.endorsementType !== endorseTypes.cancel && policy.expiryDate) {
    const date = new DateObject(policy.expiryDate);
    if (date.isValid === true) {
      date.setHour(23);
      date.setMinute(59);
      date.setMillisecond(59);
      const bufferExpiry = date.add(travelClaimBufferDurationDays, "d");
      result = bufferExpiry.valueOf() <= Date.now();
    }
  }
  return result;
};
