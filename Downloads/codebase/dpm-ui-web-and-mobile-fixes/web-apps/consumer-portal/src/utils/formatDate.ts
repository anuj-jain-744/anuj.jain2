import { dateFormats, validateDaysToExpiry } from "constant";
...
export const isPastDateTime = (
  dateToCheck: string | DateObject = "",
  format: string = dateFormats.date
): boolean | null => {
  if (dateToCheck) {
    const date = new DateObject({
      date: dateToCheck,
      format: format.length === dateToCheck?.length ? format : undefined,
    });
    if (date.isValid === true) return date.valueOf() < Date.now();
  }
  return null;
};
