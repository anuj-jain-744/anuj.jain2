...
export const PHQuoteBuyLayout: React.FC<LayoutProps> = ({
  leftPanel,
  rightPanel,
  languageData,
  navigateTo,
  ...
  const { declareOptions } = commonKeywords;
  ...
  const {
    init,
    getRiskID,
    userAddressFormSelection,
    updateDeclarationItems,
    renewalPayloadRequest,
    contentBenefitsDeclaration,
    additionalBenefits,
    makeAdditionalBenefits,
  } = usePropertyPayload();
  const { makeApiCall: uploadPhotos } = useUploadFile();

  const { back, selectCoverage, review_quotation, get_quote_pay } = homeConfig;

  const { saveRedisData } = useSaveRedisData();

  const uniqueKey =
    propsData?.ownerId?.toString() + propsData?.mobileNumber?.toString() + HOME;
  ...
  const { makeApiCall: redisKey, data: redisData } = useApiCall<
    { key: string },
    undefined
  >(6, `${JAVA_API_ROUTES?.redisGetValue}/${uniqueKey}`, "get");
  ...
  const {
    makeApiCall: viewPoilcy,
    data: viewPoilcyData,
    errors: viewPoilcyError,
    isLoading: viewPoilcyLoading,
  ...
  const {
    ...
    data: directDraft,
    errors,
    isLoading: directDraftLoading,
  } = useApiCall(10, apiRoutes.homeDirectDraft, "post");

  // set reject quote hooks for reject quotation
  const {
    makeApiCall: rejectQuote,
    data: rejectQuoteData,
    errors: rejectQuoteError,
    isLoading: rejectQuoteLoading,
  } = useApiCall(10, apiRoutes.homeRejectQuote, "post");

  // set calculate premium hooks
  const { handleCalculatePremium, isError, isLoadingCalculatePremium, data } =
  ...
  const makeDeclarationItems = (
    data: [],
    declarationItems: { itemDescription: string; sarValue: string }[]
  ) => {
    data.forEach((item: { category: string; value: string }, index: number) => {
      const checkInItem = declarationItems.findIndex(
        (value) => value.itemDescription === item.category
      );
      if (checkInItem === -1 && item.category && item.value) {
      ...
      if (!item.category && !item.value) {
        data.splice(index, 1); // remove the item if both category and value are empty
      }
    });
  ...
  const updatePremiumPayload = async (
    requestPayload,
    policyStartDate: string | null
  ) => {
    if (selectedContetBenefits.length) {
      const policyCoverage = requestPayload.policyRisk[0].policyCoverage;
      const declarationIndex = policyCoverage.findIndex(
        (data) => data.coverageCode === languageData?.defaultCoverageCode
      );
  ...
  const handleLinkClick = async () => {
    if (leftStep === 1) {
      ...
      const payloadScheme = userAddressFormSelection(
        payload,
        formAddressSelection,
        propsData,
        showPropertyMap,
    ...
    if (leftStep === 2 && repairTypeSelected) {
      ...
      if (!homePolicyRenewal) {
        ...
        saveRedisData("multiValue", savedRedisData, 3);
      }
    }
    if (
      leftStep === 3 &&
      ...
      const riskID = getRiskID(
        homePremiumResponse,
        repairTypeSelected,
        languageData?.defaultCoverageCode
      );
      const payloadScheme = updateDeclarationItems(
        requestPayload,
        declaration,
        declare,
        riskID
  ...
  const setPolicyRenewalChanges = async (policyData) => {
    ...
    const propertyOptions =
      languageData?.property_form_values_string?.type_form?.option;
    const propertyNo = Number(policyForm?.ownerOrTenant) - 1;
    const coverageValue =
      policyForm?.planCode?.replace(/\s+/g, "").toLowerCase() || null;
    ...
    setFormAddressSelection((prev) => ({
      ...
      propertyType: {
        activeIndex: propertyNo,
        activeLabel: propertyOptions[propertyNo],
      },
    }));

    const renewalContentBenefits = contentBenefitsDeclaration(
    ...
    setHomePolicyRenewal({
      ...
      contentBenefits: renewalContentBenefits,
      selectedBenefits: additionalBenefits(
        policyRisk.policyCoverage,
        languageData
      ),
    ...
    const renewalPremiumInfo = {
      coverageType: policyForm?.planCode?.replace(/\s+/g, "").toLowerCase(),
      selectedBenefits: makeAdditionalBenefits(
        policyRisk.policyCoverage,
        languageData
      ),
      contentBenefits: renewalContentBenefits,
    ...
    await handleCalculatePremium(payloadScheme, renewalPremiumInfo);
  };
  ...
  useEffect(() => {
    if (viewPoilcyError) {
      ...
      setApiErrorMessage({
        ...
        description:
          viewPoilcyError.messages?.message_en ??
          languageData?.something_went_wrong,
      });
    }
  }, [viewPoilcyError]);

  // convert the array structure from payload request to content benefits functionality updated
  function renameBenefits(
    benefits: { itemDescription: string; sarValue: string }[]
  ) {
  ...
  useEffect(() => {
    if (!data || isError || isLoadingCalculatePremium) return;
  ...
  const handleLeftStepTwo = () => {
    if (coverageType) {
      setStepValue(homePolicyRenewal ? 1 : 2);
      setLeftStep(3);
    } else if (homePolicyRenewal) {
      // Policy renewal case
      ...
      if (homePolicyRenewal?.selectedBenefits) {
        setSelectedBenefits(homePolicyRenewal?.selectedBenefits);
      }
    }
  };
  ...
  useEffect(() => {
    if (isError) {
      ...
      setApiErrorMessage({
        ...
        description:
          isError.messages?.message_en ?? languageData?.something_went_wrong,
      });
    }
  }, [isError]);
  ...
  useEffect(() => {
    const contentBenefits = selectedContetBenefits?.filter(
      (item) =>
        (!item?.category && item?.value) ||
        (item?.category && !item?.value) ||
        (item?.errors && (item?.errors?.category || item?.errors?.value))
    );
    ...
    if (leftStep === 3) {
      ...
      const mandatoryDecalredItems = declare?.filter((item) => {
        const selectedItems =
          declaration && declareOptions[declaration[item.key]];
  ...
  const updatePlanCodeToPayload = useCallback(
    (requestPayload: []) => {
      const coverageType: string = repairTypeSelected
        .replace(/\s+/g, "")
        .toLowerCase();
  ...
  // if the quotation is already generated, then call the reject quote API to reject the existing quote
  useEffect(() => {
    if (rejectQuoteData && !rejectQuoteLoading) {
      const payloadScheme = updatePlanCodeToPayload(requestPayload);
      makeApiCall(payloadScheme);
    }
    if (rejectQuoteError) {
      setApiErrorMessage({
        title: rejectQuoteError?.name || languageData?.internal_server_error,
        description:
          rejectQuoteError.messages?.message_en ??
          languageData?.something_went_wrong,
      });
      setShowAlertModal(true);
    }
    ...
    if (directDraft) {
      ...
      if (!homePolicyRenewal) {
        saveRedisData(
          "multiValue",
          {
            quoteNumber: directDraft?.quoteNo,
            expiryDate: directDraft?.quotationExpiryDate,
            referanceNumber: directDraft?.quoteReferenceNo,
          },
          3
        );
      }
      ...
      navigateTo &&
        navigateTo("/product/insurance-payment", {
          quoteData: directDraft,
          userInfo: propsData,
          productCode: PRODUCTCODE_HOME,
        });
    }
    if (errors) {
      setApiErrorMessage({
        ...
        description:
          errors.messages?.message_en ?? languageData?.something_went_wrong,
      });
      setShowAlertModal(true);
    }
  }, [directDraft, errors]);
  ...
  return (
    <div className="quote-container">
      ...
      {propsData?.mobileNumber && callGenerateOtp && (
        <OTPWrapper
          ...
          handleSuccessValidation={async () => {
            ...
            if (redisData?.quoteNumber) {
              // if the quote number is available, then reject the existing quote
                await rejectQuote({
                  quoteReferenceNo: redisData?.quoteNumber,
                });
            } else {
              const payloadScheme = updatePlanCodeToPayload(requestPayload);
      ...
      <div className="rightPanel">
        <PanelRight>{rightPanel}</PanelRight>
      </div>
      <BackFooter
        classes="home-footer"
        isBackBtnDisabled={
          (propsData?.policyNumber && leftStep === 2) || leftStep === 1
        }
