...
export function useCoveragePlanApi(setLeftStep) {
  ...
  const handlecoveragePlanSelf = async (
    ...
    const dob = requestPayloadCoveragePlan.policyCustomer.dateOfBirth.toLocaleString();
    const proposerAge = getFullAge(dob.split('-').reverse().join('-')
      , '-');
    if (proposerAge > 80) {
      // Fetch only Europe data for users > 80
      await apiselfeurope(payloadselfeurope);
    } else {
      // Fetch all data for users <= 80
      await Promise.all([
  ...
  useEffect(() => {
    if (dataselfeurope) {
      setCoverageplanselfworldwideeurope(dataselfeurope);
      setCoverageplanselfworldwide(null);
      setCoverageplanselfworldwideusa(null);
      setIsCoveragePlanData(true);
      setLeftStep(2);
    }
    if (dataselfworldwide && dataselfworldwideusa && dataselfeurope) {
      setCoverageplanselfworldwide(dataselfworldwide);
      setCoverageplanselfworldwideusa(dataselfworldwideusa);
      setCoverageplanselfworldwideeurope(dataselfeurope);
      setIsCoveragePlanData(true);
      setLeftStep(2);
    }
