...
describe("Promocode Component", () => {
  const mockProps = {
    ...
    languageData: {
      ...
      premium_breakdown: [], // Added required property
      internal_server_error: "Internal Server Error", // Added required property
    },
  ...
  it("updates promo code value on input change", () => {
    ...
    expect((input as HTMLInputElement).value).toBe("DISCOUNT50");
  });
  ...
  it("calls API when apply promo code button is clicked", async () => {
    ...
    useApiCall.mockReturnValue({ setData: jest.fn(),  makeApiCall: mockApiCall, data: null });
    render(<Promocode {...mockProps} />);
  ...
  it("displays success message when coupon is applied", () => {
    useApiCall.mockReturnValue({ setData: jest.fn(), data: null });
