...
import { callAPI, getFullUrl, sanitizeHtml, RootState } from "@dpm/shared-module";
...
import { INTERNAL_SERVER_ERROR, SOMETHING_WENT_WRONG, TRAVEL_POLICY_TYPE, HOME, coverageTypeIdMap, coveragePlanTypeIdMap } from "constant";
import { AlertBox } from "components/AlertBox";
import { useRef } from "react";
import { TRAVEL_COVERAGE_TYPE, docType } from "../../constant";
...
function TravelSuccessLeft({ status, data, flag = false, cancelPolicyData, startDate, address }: TravelSuccessLeftProps) {
  ...
  useEffect(() => {
    const fetchData = async () => {
    try {
      if (fetchref.current && refundPolicy?.productCode !== HOME) {
        await getIssuePolicyDetails();
  ...
  const { data: policyDocuments, error: policyDocError, isError:isPDFError, isLoading: isPolicyDocLoading, refetch } = 
  usePolicyDocuments({ policyNo: refundPolicy?.policyNo, docType: docType.Policy.Policy_Schedule.code });
  const { processPDFs } = useDownloadPDF();
  useEffect(() => {
    if (policyNumber) {
      refetch();
    }
  }, [policyNumber, refetch]);

  const handleDownloadRefundReceipt = useCallback(() => {
    // need to implement the functionality to download the refund receipt, once the API is ready
  }, []);

  const handleDownloadPolicy = useCallback(async () => {
    try {
      await processPDFs(policyDocuments, { format: "pdf", autoDownload: true });
    } catch (err) {
      console.error("Error processing PDFs:", err);
      setApiErrorMessage({
        title: "",
        description: policyDocError?.messageEn ?? ""
      });
  ...
  const renderPriceTravel = () => {
    switch (travelcoverage) {
      ...
      case coverageTypeIdMap.worldwide: {
        ...
        } }else if (travelcoverageTypeCode === coveragePlanTypeIdMap.type3) {
          return {
      ...
      case coverageTypeIdMap.europe: {
        ...
        } else if (travelcoverageTypeCode === coveragePlanTypeIdMap.type5) {
          return {
...
<>
  <Card className="policy-cancellation-new">
    <div className="header">
      <div className="policy-number-view">
        <div className="policy-cancel-number-heading-text walaa-regular-400">
          {data?.policy_number}
        </div>
        <div className="policycancel-number walaa-medium-500">
          {refundPolicy?.policyNo ? refundPolicy?.policyNo.toLocaleUpperCase() : ''}
        </div>
        <div className="policynumber-view">
          <div className="curved-blue-boxText walaa-regular-500">
      ...
      <div className="policynumber-container">
        <div className="product-logo">
          {
            refundPolicy?.productCode === HOME ?
              <img src={Home} className="product-logos" alt="Product-logos" /> :
              <img src={travelIcon} className="product-logos" alt="Product-logos" />
          }
        </div>
      </div>
    </div>

    <div className="updatebody-content">
    {refundPolicy?.productCode === HOME && 
      <div>
        <div className="property-text walaa-regular-400">{data?.property}</div>
        <div className="address-text walaa-regular-500">
          {[
              address?.streetName,
              address?.city,
              address?.postCode,
        ...
        <div className="box">
          <div className="boxcontent">
            <div className="package-content">
              <div className="package-heading-left walaa-regular-400">
              {data?.start_date}
              </div>
              <div className="package-value-letf walaa-medium-500">
                 {formatDate(startDate || today)}
              </div>
            </div>  

            <div className="package-content">
              <div className="package-heading-right walaa-regular-400">
              {data?.cancellation_date}
              </div>
              <div className="package-value-right walaa-medium-500">
                {todayDate}
              </div>
            </div>
          </div>
        ...
        <div className="box">
          <div className="boxcontent">
            ...
            <div className="package-content">
              ...
              <div className="package-value-right walaa-medium-500">
                 {getAmountWithIcon(Math.abs(refundPolicy?.cancelRefund) || 0)}
              </div>
            </div>
    ...
    <div className="update-bottom-links">
        <div className="linkcontainer">
          <div className="download-wrapper-new">
            <img src={Download} alt="Download Icon" />
          </div>
          <div className="link-content walaa-regular-400" onClick={handleDownloadPolicy}>
  ...
  <>
  <Card className="policynote-section">
      <div className="footernote">
        <div className="update-note walaa-medium-500">{data?.note}</div>
        <div className="update-note-content walaa-regular-400">
  ...
  const PolicySucessCard = (
    <Card className="policy-receipt-policy-cancellation">
      ...
      <div className="header">
        <div>
          <div
            className="policy-number-heading walaa-regular-400"
            data-testid="tr-popicy-no"
          >
            {data?.policy_number}
          </div>
          ...
          <div className="coverage-plan walaa-medium-500">
            {formattedTravelStartDate} - {formattedTravelEndDate}
          </div>
        </div>
        ...
        <div className="policy-number-container">
          ...
          <div className="coverage-plan walaa-medium-500">
            {renderPriceTravel()?.type}
          </div>
        </div>
      </div>
      {status && (
        <div className="space-usercard">
          <UserCard languageData={data as LanguageData | null} />
        </div>
      )}
      <div className="body-content">
        <div className="top-table">
            ...
            <div className="box-content">
              ...
              <div className="package-content">
                ...
                <div className="package-value walaa-medium-500">
                  {travellerType.charAt(0).toUpperCase() +
                    travellerType.slice(1)}
                </div>
              </div>
              <div>
                <img src={Line} />
          ...
          <div className="activeTagContainer">
            <div className="activeTagText walaa-regular-400">
              {data?.confirmed}
            </div>
          </div>
        </div>
        <div className="bottom-links">
          <div className="link-container">
            <div>
              <img src={Download} onClick={handleDownloadPolicy} />
            </div>
            ...
            <div className="links-content walaa-regular-400">{data?.share}</div>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="left-card-container-travel">
      ...
      <div className="bottom-links walaa-medium-500">
        ...
        <div>
          <ThemeButton
            title={status ?  data?.view_my_dashboard ?? "" : data?.go_to_dashboard ?? ""}
