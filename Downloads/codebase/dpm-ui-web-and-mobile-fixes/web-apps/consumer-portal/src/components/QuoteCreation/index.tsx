...
export default function QuoteCreation({
  ...
  useQuotationDocuments({ quoteReferenceNo: quotationNum, docType: docType.Quotation.Quotation_Letter.code });

  const { processPDFs } = useDownloadPDF();
  const handleDownload = useCallback(async () => {
      try {
        await processPDFs(quotationDocuments, { format: "pdf", autoDownload: true });
      } catch (err) {
