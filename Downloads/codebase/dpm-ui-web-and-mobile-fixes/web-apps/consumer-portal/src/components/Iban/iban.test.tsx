...
describe("Iban Component", () => {
  ...
  it("uploads valid file and shows uploaded state", async () => {
    ...
    const fileInputs = screen.getAllByLabelText(
      /Upload Cheque Leaf|Upload Other Documents/i
    );
  ...
  it("shows toast error for invalid file type", async () => {
    ...
    const invalidFile = new File(["hello"], "test.exe", {
  ...
  it("toggles accordion open and closed", () => {
    ...
    expect(toggleButton.querySelector("img")?.getAttribute("src")).toContain(
      "test-file-stub"
    );
