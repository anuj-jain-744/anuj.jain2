...
const CoverageOptions: React.FC<CoverageSelectorProps> = ({
  ...
  const [coveragetypeselected, setCoverageTypeSelected] = useState<
    string | null
  >(null);
  ...
  if (coveragedata) {
    coveragedata.coverageTypes.sort((a, b) =>
      a.coverageType.localeCompare(b.coverageType)
    );
  }
  ...
  const replaceName = (text: string, SelectedPlan: string): string => {
    const selectedPlanName = TravelData.travel_coverage_plan.find(
      (plan) => plan.codeid === SelectedPlan
    )?.title;
  ...
  const getToolTipPlanTypes = () => {
    if (!coveragedata?.coverageTypes || !tooltipOptions) return [];
    const coverageTypeTitles = coveragedata.coverageTypes.map(
      (type) => type.coverageType
    );
    return TravelData?.tooltip.filter((tooltip) =>
      coverageTypeTitles.includes(tooltip.title)
    );
  };
  const commonCoverageTypes = getToolTipPlanTypes();
  return (
    <div className="coverage-option-container mt-3">
      <div className="coverageplan-text">
        {TravelData?.coverage_type_description &&
          replaceName(
            TravelData?.coverage_type_description,
            coveragePlanSelected
          )}
      ...
      <div className="compare_benefit_main">
        <h5 className="coverage-option-text">
          {TravelData.select_plan_type}
          {coveragedata?.coverageTypes?.length > 1
            ? ` (${coveragedata?.coverageTypes?.length})`
            : ""}
          <div className="icon-container flex-row ms-1">
            <InfoOutlinedIcon
        ...
        {coveragedata?.coverageTypes?.length > 1 && (
          <div
            onClick={() => handleShowModal(travelcoverage)}
            role="button"
            tabIndex={0}
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                e.preventDefault();
                handleShowModal(travelcoverage);
              }
            }}
          >
      ...
      <div className="row mt-3">
        {coveragedata &&
          coveragedata.coverageTypes.map(
            (coverageType: CoverageType, index: number) => {
              const isFirst =
                index === 0 && coveragedata.coverageTypes.length === 2;
              const isLast =
                index === 1 && coveragedata.coverageTypes.length === 2;
              const onRecord =
                index === 0 && coveragedata.coverageTypes.length === 3;
              const onRecordLast =
                index === 2 && coveragedata.coverageTypes.length === 3;
              const TotalLength = coveragedata.coverageTypes.length === 3;
              const singleRecord = coveragedata.coverageTypes.length === 1;
              const tooltiptype =
                coverageType.coverageType === "Traveler"
                  ? "Traveller"
                  : coverageType.coverageType;

              const singleRecordClass = singleRecord
                ? "border-first border-last"
                : "border-none";

              const lastRecordClass =
                isLast || onRecordLast ? "border-last" : singleRecordClass;

              const singleRecordColumnClass = singleRecord
                ? "col-md-12"
                : "col-md-6";

              return (
                <div
                  key={"coverageType_" + index}
                  className={`coverage-col ${
                    isFirst || onRecord ? "border-first" : lastRecordClass
                  }  ${index == 1 ? "blue-bg" : "pearl-bg"} ${
                    TotalLength ? "col-md-4" : singleRecordColumnClass
                  }`}
                >
                  <div className="coverage-block">
                    <div className="coverage-option-check">
                      <ThemeRadioCheckbox
                        ...
                        checked={
                          travelcoverageType === coverageType.coverageType
                        }
                        onChangehandler={(
                          event: React.ChangeEvent<HTMLInputElement>
                        ) =>
                  ...
                  <div className="price-wrapper">
                    <h4 className="coverage-type-price">
                      {getAmountWithIcon(
                        coverageType?.purchasedCoverage?.[0]?.premiumInfo
                          ?.annualPremium,
                        "coverage-price"
                      )}
                    </h4>
                    <button
                      type="button"
                      className="view-benefit-link"
                      onClick={() =>
                        handleShowViewModal(
                          travelcoverage,
                          coverageType.coverageType.trim() ===
                            TRAVELER_COVERAGE_COMPARE
                            ? TRAVELER_COVERAGE_TYPE
                            : coverageType.coverageType.trim()
                        )
                      }
                    >
        ...
        >
          <Modal.Header>
            <Modal.Title
              ...
              className="custom_tooltip_title"
            >
              {TravelData?.coverage_types}
            </Modal.Title>
            <CloseIcon
              data-testid="img-role"
              className="compare_modal-close-icon"
              onClick={closeTollTip}
            />
          </Modal.Header>
          <div className="actual-content">
            {commonCoverageTypes?.map((info, index) => (
              <CoverageTypeInfo
                key={"tooltipOption_" + index}
                title={info.title}
      ...
      {showViewModal && (
        <ViewBenefitPopup
          ...
          plan={visibleplans}
          coveragetypeselected={coveragetypeselected}
        />
      )}
    </div>
  );
