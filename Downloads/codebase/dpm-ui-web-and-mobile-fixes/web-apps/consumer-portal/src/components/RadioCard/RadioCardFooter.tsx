...
const WorldwideexceptFooter = ({ TravelData }: TravellerTypeSelect) => {
  ...
  const wwUsaVal1 = dataCoverageplanselfworldwideusa?.data?.coverageTypes?.[0]?.purchasedCoverage?.[0]?.premiumInfo?.annualPremium;
  const wwUsaVal2 = dataCoverageplanselfworldwideusa?.data?.coverageTypes?.[1]?.purchasedCoverage?.[0]?.premiumInfo?.annualPremium;
  ...
  return (
    <React.Fragment>
      ...
      <div className="d-flex align-items-center">
        <div>
        <span className="footer-denomination">
            {   wwUsaVal1 > wwUsaVal2 ? getAmountWithIcon(wwUsaVal2,'walaa-medium-500 footer-amount') : getAmountWithIcon(wwUsaVal1,'walaa-medium-500 footer-amount')}&nbsp;</span>
        </div>
...
const wwselfVal1 = dataCoverageplanselfworldwide?.data?.coverageTypes?.[0]?.purchasedCoverage?.[0]?.premiumInfo?.annualPremium;
const wwselfVal2 = dataCoverageplanselfworldwide?.data?.coverageTypes?.[1]?.purchasedCoverage?.[0]?.premiumInfo?.annualPremium;
const leastWwSelfValue = (wwselfVal1 > wwselfVal2) ? wwselfVal2 : wwselfVal1;
  return (
    <React.Fragment>
      <div className="walaa-medium-500">{TravelData?.starting_from}</div>
      <div className="d-flex align-items-center">
        <div>
        {travellerType === "1" ? <span className="footer-denomination">{getAmountWithIcon(dataCoverageplanfamilyworldwide?.data?.coverageTypes?.[0]?.purchasedCoverage?.[0]?.premiumInfo?.annualPremium,'walaa-medium-500 footer-amount')}&nbsp;</span>: <span className="footer-denomination">{getAmountWithIcon(leastWwSelfValue,'walaa-medium-500 footer-amount')}&nbsp;</span>} 
        </div>
...
const EuropeFooter = ({ TravelData }: TravellerTypeSelect) => {
  ...
  return (
    <React.Fragment>
      ...
      <div className="d-flex align-items-center">
        <div>
        <span className="footer-denomination">{getAmountWithIcon(dataCoverageplanselfeurope?.data?.coverageTypes?.[0]?.purchasedCoverage?.[0]?.premiumInfo?.annualPremium,'walaa-medium-500 footer-amount')}&nbsp;</span>
        </div>
        <div className="walaa-regular-400 footer-vat"> + {compensationTypeCardFinalVAT(
