...
function MoroorReport({ changeHandler, checkHandler }: typeCompreUploadFiles) {
  //MoroorReport
  const [selectedFileMoroorRep, setSelectedFileMoroorRep] = useState<{
    file: File | null;
    base64: string | null;
  }>({ file: null, base64: null });
  const fileInputRefMoroorRep = useRef<HTMLInputElement>(null);
  ...
  const handleFileUpload = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file) {
      ...
      if (validTypes.includes(file.type) && file.size <= 3 * 1024 * 1024) {
        ...
        if (fileInputRefMoroorRep.current) {
          fileInputRefMoroorRep.current.value = "";
        }
      }
    }
  };
  ...
  useEffect(() => {
    selectedFileMoroorRep?.file !== null
      ? checkHandler("Moroor Report", true)
      : checkHandler("Moroor Report", false);
  }, [selectedFileMoroorRep]);

  // Delete file and clear input
  const deleteFileMoroorRep = () => {
    setSelectedFileMoroorRep({ file: null, base64: null });
    if (fileInputRefMoroorRep.current) {
      fileInputRefMoroorRep.current.value = "";
    }
    changeHandler(null);
                    ...
                    <React.Fragment>
                      <div className="d-flex align-items-center walaa-medium-500 py-2">
                        <div className="upload-title">
                          {icnFactory(selectedFileMoroorRep?.file?.type)}
                          {selectedFileMoroorRep?.file?.name}
                          <span className="mx-1">
                            {selectedFileMoroorRep?.file?.size}&nbsp;KB
                          </span>
                        </div>
                        <div
                          title={language?.delete_document}
                          className="upload-white-icon d-flex align-items-center"
                        >
                          <HighlightOffIcon
                            color="error"
                            onClick={deleteFileMoroorRep}
                          />
                        </div>
                      </div>
                    </React.Fragment>
