...
//      Amount: Math.round(Number(quoteData?.premiumDue).toFixed(2) * PAYMENT_INFO.paymentDirect),
  ...
  useEffect(() => {
    if (cardType !== undefined && paymentMethods && quoteData) {
      const cardName = getMethodType(cardType ?? 0);
      const cardData = paymentMethods.find((method) => method.methodDesc?.toLowerCase() === cardName?.toLowerCase() && method.providerDesc?.toLowerCase() === PAYMENT_INFO.paymentMethods.CARD_PAY);
