...
export default function StcURPayPayment({
  ...
  useEffect(() => {
    if (intianlData && progressData && !initalError && !progressError) {
      const paymentMobileNumber = paymentDets?.mobileNumber?.replace(/^0/, "966");
      setPaymentDets((prevValue) => ({
        ...prevValue,
        loading: false,
        otpRequestPayload: {
          ...
          amount: Math.round(Number(quoteData?.premiumDue).toFixed(2) * PAYMENT_INFO.paymentDirect),
          mobile: `%2B${paymentMobileNumber}`
        },
  ...
  const enabledPay = () => {
    return !paymentDets?.mobileNumber || paymentDets?.mobileNumber?.length <= 9;
  }
  ...
  return (
    <div className="card-options">
      ...
      {paymentDets?.otpRequestPayload && <OTPWrapper
        ...
        paymentType={paymentInfo[payType].payName}
        languageData={{
      ...
      <div className="card-no-input">
        ...
        <ThemeButton
          ...
          buttonTitle={paymentInfo[payType].buttonTitle}
