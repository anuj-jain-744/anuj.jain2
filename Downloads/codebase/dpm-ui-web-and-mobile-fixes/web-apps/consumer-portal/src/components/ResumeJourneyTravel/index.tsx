...
function ResumeJourney({
  ...
  const handleContinue = async () => {
    if (redisData) {
      ...
      setTravelDateRange(
        (data?.travelDateRange ?? []).map(timestamp => new DateObject(timestamp))
      );
      ...
      if (data?.currentStep === 2) {
        const travelRange = data?.travelDateRange?.map(timestamp => new DateObject(timestamp))
        const [travelStartDate,] = travelRange;
        const formattedTravelStartDate = formatDateObjectTo(travelStartDate, DATE_FORMATS["YYYY-MM-DD"]); 
        ...
        if (coveragePlanPayload !== null) {
          ...
          coveragePlanPayload.travelDateRange = travelRange;         
