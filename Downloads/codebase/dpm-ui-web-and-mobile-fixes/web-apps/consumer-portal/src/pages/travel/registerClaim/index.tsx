...
const RegisterClaim = ({
  ...
  const [isContactValid, setIsContactValid] = useState<{
    [key: string]: boolean;
  }>({ iBAN: false, mobilenum: true, emailId: true });
  ...
  const isValidIban = useMemo(
    () =>
      Object.values(isContactValid).every((item) => item === true) &&
      iBANDetails.mobile &&
      iBANDetails.iBAN,
    [isContactValid, iBANDetails]
  );
  ...
  const handleContactChange = (
    ...
    setIsContactValid({ ...isContactValid, [name]: isIBAN });
    switch (name) {
      case "iBAN":
        bankName &&
          setIBANDetails((prevDetails) => ({
            ...prevDetails,
            bankName: bankName ?? "",
          }));
        break;
      case "mobilenum":
        setIBANDetails((prevDetails) => ({
          ...prevDetails,
          mobile: value !== undefined ? value : prevDetails.mobile,
        }));
        break;
      case "emailId":
        setIBANDetails((prevDetails) => ({
          ...prevDetails,
          emaiId: value !== undefined ? value : prevDetails.emaiId,
        }));
        break;
    }
  };
  ...
  useEffect(() => {
    setPlace("");
    setEstimations([]);
    setIsContactValid({ iBAN: false, mobilenum: true, emailId: true });
    setIBANDetails({
      bankName: "",
      emaiId: "",
      iBAN: "",
    ...
    try {
      ...
      if (
        ...
        toast.error(response?.errors[0]?.messages?.message_en, {
          ...
          autoClose:
            parseInt(langData.product.toaster_timeout) || TOAST_AUTOCLOSE_TIMER,
        });
      }
    } catch (error) {
      setEstimations([]);
  ...
  const handleContinueBtn = () => {
    const selectedPolicyDetails = policies.find(
      (item) => item.policyNo === selectedPolicyNumber
    );
    ...
    const expiryPlus28 = new Date(expiryDate.getTime() + add28Days);
  ...
  const handleBackButton = () => {
    setDateOfLoss("");
    selectedPolicyNumber && policies.length !== 1
      ? (setShowCards(true), setSelectedPolicyNumber(""))
      : backBtnClickHandler();
  };
              ...
              <div className="footer-btns walaa-medium-500">
                ...
                <div>
                  {showCards ? (
                    ...
                    <ThemeButton
                      classes={
                        isValidIban && isTermsChecked && isValidEstimation
                          ? "payment-btn register-enabled"
                          : "payment-btn register-disabled"
                      }
                      isDisabled={
