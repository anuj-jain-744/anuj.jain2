...
export const useCancelPolicy = ({
  ...
  const fetchCancelPolicy = useCallback(
    async (cancelReason: string, financialEndoInd: string = "Y") => {
      setIsLoading(true);
      setError(null);
      ...
      const baseUrl =
        productCode === HOME
          ...
          : VITE_TRAVEL_POLICY_CANCELLATION_BASE_URL + "CancelPolicy";
      ...
      try {
        const responseData = await callAPI("post", baseUrl, requestBody);
        if (responseData.code === 1 && responseData.message === "SUCCESS") {
          setPolicyCancelData(responseData);
        } else {
          const errorBody = responseData?.errors[0];
