...
import {FILTER_TYPES, DEFAULT_CARD_TOSHOW, myProfile} from "../../../constant";
...
const MyPoliciesContainer = ({ hideBanner }) => {
  ...
  const [cardsToShow, setCardsToShow] = useState(!hideBanner ? DEFAULT_CARD_TOSHOW : allPolicies.length);
    ...
    return visibleCards?.map((card, index) => {
      ...
      switch (card.type) {
        ...
        default:
          return (
            <PoliciesCard
              key={card?.quoteNo as string || index}
              policy={card}
  ...
  return (
    <div className="dashboard-container">
      <div className={`${hideBanner ? "policies-spacing" : ""} policies-section`}>
        {showAlert && (
          <AlertPopUp
            varaint={alertMessage.variant}
            title={languageData?.add_your_email_id_to_the_profile}
            message={languageData?.please_add_your_email_id}
        ...
        <div className="policies-list-container">
          ...
          {!hideBanner && !showMore && totalItemsForFilter > DEFAULT_CARD_TOSHOW && (
            <div className={""}>
              <ExpandSection
        ...
        {!hideBanner && <MyRequest />}
        {!hideBanner && <WalaaOfferings />}
        {!hideBanner && <CarouselImages />}
      </div>
      <div className={`${hideBanner ? "ads-section-spacing" : ""} ads-section`}>
        <ContactCard />
        <PersonalDashboardAdd hideBanner={hideBanner} />
        {!hideBanner && <EnhanceExperience />}
      </div>
    </div>
  );
};
