...
const getFormattedDate = () => {
  ...
  const handleNavigate = (type: string | undefined) => {
    ...
    if (type === REQUEST_TYPES.QUOTATION) {
      ...
      data && navigate("/PolicyService/History", {
        state: { data },
      });
    }
  };
    ...
    <div className={style.frameContainer}>
      ...
      <div className={style.productContentContainer}>
        <div className={style.typeContainer}>
          <div className={style.typeLabelContainer}>{languageData?.request_id || 'Request ID'}</div>
          <div className={style.typeValueContainer}>{getType()}</div>
        </div>
        <div className={style.typeContainer}>
          <div className={style.typeLabelContainer}>{languageData?.request_type || 'Request Type'}</div>
          <div className={style.typeValueContainer}>{cardType}</div>
        </div>
        <div className={style.statusContainer}>
          <div className={style.statusLabelContainer}>
            {languageData?.status}
          </div>
        ...
        <div
          ...
          onClick={() => handleNavigate(cardType)}
          role="button"
          tabIndex={0}
          onKeyDown={(e) => {
            if (e.key === "Enter") {
              e.preventDefault();
              handleNavigate(cardType);
            }
          }}
        >
          <div className={style.linkContainer}>
            <div
              className={
                isStatusDisable() ? style.isDisabled : style.linkTextContainer
              }
            >
              <div
                className={
                  isStatusDisable() ? style.isDisabled : style.linkText
                }
              >
                {getNavigationText()}
              </div>
            ...
            <img
              src={isStatusDisable() ? disableArrow : actionArrow}
              alt="arrow"
            />
          </div>
          {!isOpen && (
            <div className={style.dateContainer}>
              ...
              >{`${languageData?.last_updated_on}:`}</div>
