...
const RegisterClaim: FC<PolicyCardProps> = ({
  claimData,
  languageData,
  backBtnClickHandler,
  ...
  useEffect(() => {
    if (nonMotorFNOLData) {
      const coverageList =
        nonMotorFNOLData?.subClaimInfo?.coverageInfoList || [];
  ...
  useEffect(() => {
    if (nonMotorRegistrationError) {
      setApiErrorMessage({
        title:
          nonMotorRegistrationError?.name ??
          languageData?.internal_server_error,
        description:
          nonMotorRegistrationError?.messages?.message_en ??
  ...
  useEffect(() => {
    if (nonMotorRegistrationData) {
      const name =
        currentLanguage === ar
          ? viewPolicy?.policyCustomer[0]?.customerNameArabic
          : viewPolicy?.policyCustomer[0]?.customerNameEnglish;
      const address = displayHouseAddress(
        viewPolicy?.policyLob[0]?.policyRisk[0],
        currentLanguage,
        ar
      );
      const claimData = {
        ...
        policyHolder: name?.trim()
          ? name?.trim()
          : nonMotorFNOL?.claimInfo?.policyHolderName,
  ...
  const handleSuccessValidation = () => {
    ...
    for (const item in coverageList) {
      ...
      const isEstimated = estimatedValues.findIndex(
        (value) => Number(data.coverageCode) === Number(value.causeOfLossId)
      );
      if (isEstimated !== -1) {
        ...
        <div className="home-claim-register-details">
          ...
          <AlertBox
            ...
            showAlertModal={Boolean(
              apiErrorMessage.title && apiErrorMessage.description
            )}
          ...
          <div className="payment-footer">
            <BackFooter
              ...
              isQuoteAndBuy={true}
              stepValue={0}
              languageData={{ "step0": languageData?.submit }}
              handleBackClick={backBtnClickHandler}
