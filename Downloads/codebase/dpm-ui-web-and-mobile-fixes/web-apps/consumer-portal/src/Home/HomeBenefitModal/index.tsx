...
const homeCoveragePlans = (
  coverageTypes: CTData,
  accordians: string[],
  headers: string[],
  languageData: any,
  coveragePlanSelected: CompensationTypeKeys
) => {
...
function HomeBenefitModal({
  ...
  const plan1OptionData: Record<string, any> = {
    [plan1]: homePremiumResponse[plan1],
  };
  const price1Data = calculatePremium(plan1OptionData);
  const price1 = price1Data.minFinalPrice;

  const plan2OptionData: Record<string, any> = {
    [plan2]: homePremiumResponse[plan2],
  };
    ...
    >
      ...
      <div className="sub-heading">
        <div className="sub-sub-heading">
          <div className="coverage walaa-medium-500">{headers[0]}</div>
            ...
            typeof item.type === "object" ? (
              item.key && openAccordion.indexOf(item.key) !== -1 ? (
                item.type.map((subitems, index) => (
                  <CoverageItem
                    key={"coverageItem_" + index}
                    triggerViewBenefit={triggerViewBenefit}
                    items={subitems}
