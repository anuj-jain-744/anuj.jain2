...
export const fetchClaimDocuments = async ({
  ...
  if (!claimNo || !subClaimNo || !transType) {
    throw new Error("Policy does not have any claim. Please check the claim number and sub-claim number.");
  }

  const requestBody = { docType, claimNo, subClaimNo, transType };
  try {
