...
interface ClaimEstimateDocumentUploadProps {
    index: number;
    estimate: { [key: string]: string | number | boolean };
...
const ClaimEstimateDocumentUpload: React.FC<ClaimEstimateDocumentUploadProps> = ({
    ...
    const allDocFields: { key: string; label?: string }[] = [
        { key: "policeReport", label: languageData?.police_report },
        { key: "stolenItemList", label: languageData?.stolen_item_list },
        { key: "purchaseInvoice", label: languageData?.purchase_invoice }
    ];

    const visibleDocKeys = estimate.visibleDocs || allDocFields.map((f) => f.key);

    return (
        <div className="claim-estimate-upload-container">
            {allDocFields
                .filter(({ key }) => visibleDocKeys.includes(key))
                .map(({ key, label }) => (
                    <DocumentRow
                        key={key}
                        title={label || key}
                        uploadedFile={estimate[key] as File | null}
