...
import { apiRoutes, PREMIUM_DATA, commonKeywords, HOME_COVERAGE_PLANS_TYPES, JAVA_API_ROUTES, HOME, PRODUCTCODE_HOME } from "constant";
...
export const PHQuoteBuyLayout: React.FC<LayoutProps> = ({ leftPanel, rightPanel, languageData, navigateTo, prodID }) => {
  ...
  const { init, getRiskID, userAddressFormSelection, updateDeclarationItems, renewalPayloadRequest, contentBenefitsDeclaration, additionalBenefits, makeAdditionalBenefits } = usePropertyPayload();
  const { makeApiCall: uploadPhotos } = useUploadFile();

  const { back, selectCoverage, review_quotation, get_quote_pay } =
    homeConfig;
  ...
  const setPolicyRenewalChanges = async (policyData) => {
    ...
    const renewalContentBenefits = contentBenefitsDeclaration(policyRisk?.policyCoverage, languageData);
    ...
    setHomePolicyRenewal({
      ...
      selectedBenefits: additionalBenefits(policyRisk.policyCoverage, languageData),
    ...
    const renewalPremiumInfo = {
      ...
      selectedBenefits: makeAdditionalBenefits(policyRisk.policyCoverage, languageData),
      contentBenefits: renewalContentBenefits,
      type: 'renewalData',
    }
  ...
  useEffect(() => {
    const contentBenefits = selectedContetBenefits?.filter((item) => (!item?.category && item?.value) || (item?.category && !item?.value) || (item?.errors && (item?.errors?.category || item?.errors?.value)))
  ...
  return (
    <div className="quote-container">
      ...
      {propsData?.mobileNumber && <OTPWrapper
