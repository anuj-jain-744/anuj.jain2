...
import {NA, TRAVEL, endorseTypes, travelClaimBufferDurationDays} from "constant";
...
const PoliciesCard = ({ policy, navigateTo, disabled }: { policy: PolicyDetail, navigateTo: (url: string, data?: { [key: string]: unknown }) => void, disabled: boolean }) => {
  ...
  if (policy?.productCode === TRAVEL && 
    policy.endorsementType !== endorseTypes.cancel &&
    policy.expiryDate) {
    const expiryDate = new DateObject(policy.expiryDate);

    if (expiryDate.isValid) {
      const bufferedExpiry = new DateObject(policy.expiryDate).add(travelClaimBufferDurationDays, "days");
      const bufferedExpiryTimestamp = bufferedExpiry.toDate().getTime(); // in ms
      const nowTimestamp = Date.now();
      return bufferedExpiryTimestamp <= nowTimestamp;
    }
  }
  return disabled;
}, [policy, disabled]);
