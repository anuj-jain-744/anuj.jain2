...
const HamburgerMenu: React.FC<Props> = ({
  ...
  return (
    ...
    <div data-testid="mock-hamburger-menu" className={menuClassName} onMouseLeave={showErrorDialog ? showErrorDialog : onMouseLeave }>
        <div className={style.policyCardHamburgerMenuContainer}>
          {menuItems &&
            menuItems
              ...
              .map((item, index) => {
                const isDisabled =
                (item.label === languageData?.add_ons && data?.productCode === TRAVEL) || (item.label === languageData?.raise_a_claim && data?.productCode === 'RMTPL');
                ...
                >
                    ...
                    className={` ${style.navigateTextContainer} ${isDisabled ? style.disabledMenuUnderline : ''   }`}
                      onClick={(e) => {
                      if (item.label === languageData?.cancel_policy && claimNumber) {
                          e.preventDefault(); // Prevent default behavior if needed
                        setShowErrorDialog(true)
                        return ; // Block handleNavigate
                        }
                        handleNavigate(e, isDisabled);
                      }}
                    >
