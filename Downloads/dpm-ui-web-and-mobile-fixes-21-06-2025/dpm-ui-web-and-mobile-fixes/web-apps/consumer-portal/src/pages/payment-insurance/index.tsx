...
import { useApiCall, getAmountText, RootState, sanitizeHtml } from "@dpm/shared-module";
...
import { PRODUCTSAPI, TRAVEL_POLICY_TYPE, TRAVEL_TYPE, PRODUCTCODE_TRAVEL, PRODUCTCODE_HOME, ENDORSEMENT_TYPE } from "constant";
...
export default function PaymentInsurance({ transactionId, navigateTo }: Readonly<PaymentInsuranceProps>) {
  ...
  const thankYouMessage = paymentLang?.config?.thankyou_msg?.replace(
    "<<dynamic>>",
    paymentData?.productCodeTitle[productCode].replace("Insurance", "") || ""
  );
  ...
  let paymentInfo: { adminFees: number; planAmount: number; productCode: string | number; planName: string; adminFeesLabel: string } = {
    adminFees: 0,
    planAmount: 0,
    productCode: "",
    planName: "",
  ...
  if (productCode === PRODUCTCODE_HOME) { // need to add admin fees with plan amount for home
  ...
  return (
    <div className="payment-insurance">
      <IdleTimeout timeout={sessionTimer.WARNING_TIME} onTimeout={handleTimeout} />
      {sessionTimeout &&
        <>
          <ErrorDialogBox
            imgSrc={LoadingTimer}
              ...
              >
                ...
                {cmsLang && (quoteDataValue ?? endrosmentData) && paymentData && option.value === selectedPaymentOption && (
                  <PaymentOptions
                    ...
                    productCode={productCode}
                    mobileNumber={quoteDataValue?.model?.policyCustomer[0]?.mobile}
          ...
          <PanelRight>
            ...
            {processTransaction.endrosmentNo && endoPremiumBreakDown &&
              <OrderSummaryCard
                ...
                subtotal={endrosmentData?.totalAmount?.subtotal ?? endrosmentData?.subtotal}
                vatAmount={endrosmentData?.totalAmount?.vatAmount ?? endrosmentData?.vatAmount}
                totalAmount={endrosmentData?.totalAmount?.totalAmount ?? endrosmentData?.totalAmount}
                endorsementType={endrosmentData?.benefitsPremiumData.length > 0 ? ENDORSEMENT_TYPE.ADD_BENEFITS : endrosmentData?.selectManagerDriver ? ENDORSEMENT_TYPE.ADD_DRIVER : ""}
