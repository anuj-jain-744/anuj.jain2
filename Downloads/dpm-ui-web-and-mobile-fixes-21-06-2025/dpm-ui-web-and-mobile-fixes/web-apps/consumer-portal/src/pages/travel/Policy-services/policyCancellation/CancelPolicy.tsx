...
function CancelPolicy({
  ...
  const [cancelPolicyData, setCancelPolicyData] = useState<CancelPolicyData | null>(null);
  ...
  const cancel_language = policyDataObj?.productCode === HOME ?  languageData?.select_dropdown_home:  languageData?.select_dropdown_travel;
  const cmsReasons = cancel_language?.split(",");
  const dropdownItems =
    cmsReasons?.map((item: string, index: number) => {
      index = policyDataObj?.productCode === HOME ?  cancelPolicyIndex?.Home + index :  cancelPolicyIndex?.Travel + index;
  ...
  const coverageType = policyDataObj?.productCode === HOME
  ...
  const handleBankName = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    useEffect(() => {
    policyDataObj.cancelRefund = refundValue;
    policyDataObj.coverageType = coverageType;
    setRefundPolicy(policyDataObj);
    ...
    <>
      ...
      {languageData && (
        <OTPWrapper
          ...
          payload={{
            mobileNumber:
              (policyDataObj?.productCode === HOME || policyDataObj?.productCode === TRAVEL)
                ? propsData?.mobileNo
