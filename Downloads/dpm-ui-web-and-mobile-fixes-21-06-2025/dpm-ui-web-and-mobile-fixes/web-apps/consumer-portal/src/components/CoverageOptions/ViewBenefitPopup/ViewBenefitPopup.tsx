...
import yesIcon from "assets/common/success_tick_circle.svg";
import noIcon from "assets/common/danger_close_circle.svg";
...
const ViewBenefitPopup: React.FC<ViewBenefitPopupProps> = ({
  data,
  onClose,
  plan,
  ...
  const getHearderItems = (searchKey: string, headers: string[]): string[] => {
    ...
    if (searchKey && headers.length > 1) {
      ...
      const matchinHeader = headers.find((header) =>
        header.toLowerCase().includes(lowercaseSearchKey)
      );
  ...
  if (
    plan === coverageTypeIdMap.worldwide &&
    TRAVEL_PLAN_TYPE.type_family === coveragetypeselected
  ) {
    benefits = data?.family_benefits[0].benefits;
    plan1 =
      TRAVEL_PLAN_TYPE.type_family === coveragetypeselected
        ? data?.family_benefits[0].family
        : data?.family_benefits[0].family;
    ...
    plan1 =
      TRAVEL_PLAN_TYPE.type_pearl === coveragetypeselected
        ? data?.worldwide_benefits[0].pearl
        : data?.worldwide_benefits[0].traveller;
    ...
    plan1 =
      TRAVEL_PLAN_TYPE.type_pearl === coveragetypeselected
        ? data?.worldwide_except_benefits[0].pearl
        : data?.worldwide_except_benefits[0].traveller;
    coverage_accordions = data?.worldwide_except_accordians;
    headers = getHearderItems(
      coveragetypeselected,
      data?.worldwide_except_headers
    );
  } else if (plan === coverageTypeIdMap.europe) {
    benefits = data?.europe_benefits[0].benefits;
    plan1 =
      TRAVEL_PLAN_TYPE.type_europe === coveragetypeselected
        ? data?.europe_benefits[0].europe
        : data?.europe_benefits[0].schengen;
  ...
  return (
    <Modal
      ...
      size="lg"
    >
      <Modal.Header className="view_header">
        <Modal.Title data-testid="tooltip-title" className="custom_modal_title">
                ...
                {index === 1 || index === 2 ? (
                  <>
                    <div className="first-word">
                      {header.split(" ")[0] == TRAVEL_PLAN_TYPE.type_traveller
                        ? TRAVELER
                        : header.split(" ")[0]}
                    </div>
          ...
          {groupedBenefits.map((group, index) => {
            if (group.header) {
                  ...
                  <Accordion activeKey={activeAccordion}>
                    <Row className="accordion-header">
                      <Col md={6} className="benefit-cell">
                        ...
                        >
                          <span className="trvel-benefit-blod">
                            {group.header}
                          </span>
                        </Accordion.Button>
                      </Col>
                      ...
                      <>
                        {group.items.map((item, itemIndex) => (
                          <Row key={itemIndex} className="benefit-row">
                            <Col
                              md={6}
                              className={`benefit-cell browcol-one benefit-cell-first ${
                                itemIndex == 0 ? "btm-underline-top" : ""
                              }`}
                            >
                              {item}
                            </Col>
                            ...
                            <Col
                              md={6}
                              className={`benefit-cell browcol-two text-content ${
                                itemIndex == 0 ? "btm-underline-top" : ""
                              }`}
                            >
              ...
              return (
                <Row
                  key={index}
                  className="benefit-row btm-underline justify-content-center"
                >
                  <Col
                    className="benefit-cell benefit-cell-empty-left benefit-cell-first first-col txt-align"
                    md={6}
                  >
                    {group.items[0] === "-" ? "" : group.items[0]}
                  </Col>
                  <Col
                    className="benefit-cell benefit-cell-first first-col txt-align"
                    md={6}
                  >
                    {group.plan1[0]}
                  </Col>
