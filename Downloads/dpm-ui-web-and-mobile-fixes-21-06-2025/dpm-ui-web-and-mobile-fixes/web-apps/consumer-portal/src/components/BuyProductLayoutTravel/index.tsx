  ...
  const productFooter = () => {
    switch (leftStep) {
      ...
      case 1: {
        const isBtnDisabled =
          isTravelDetailValid ||
          (ownerDetailsResponseData?.age > 80 && travellerType === travelerTypeIdMap.family)  || isAddTravelerValidation;;
        return (
          <BuyProductFooter
            classNames={
    ...
    if (leftStep === 3 || leftStep === 4 || leftStep === 2) {
      ...
      const newQuery = {
        primary: {
          ...
          coverage: primaryTravelers.map((item) =>(item?.policyCoverage ?? []).map((val) => val.coverageCode)
              .sort((a, b) => a.localeCompare(b)))
        },
        adult: {
          count: travelers.length,
          coverage: travelers.map((item) =>(item?.policyCoverage ?? []).map((val) => val.coverageCode)
               .sort((a, b) => a.localeCompare(b))),
        ...
        child: {
          ...
          coverage: travelersChild.map((item) =>(item?.policyCoverage ?? []).map((val) => val.coverageCode)
               .sort((a, b) => a.localeCompare(b))),
        ...
        senior: {
          ...
          coverage: travelersSrcitizen.map((item) =>(item?.policyCoverage ?? []).map((val) => val.coverageCode)
               .sort((a, b) => a.localeCompare(b))),
