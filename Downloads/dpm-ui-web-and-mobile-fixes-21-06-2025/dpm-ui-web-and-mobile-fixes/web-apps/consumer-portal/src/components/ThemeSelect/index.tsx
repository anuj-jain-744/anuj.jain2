...
function ThemeSelect({ name, onChangehandler, isLoading, data, defaultValue }: ISelectType) {
  ...
  return (
    <div className="theme-select">
      <Form.Select data-testid="data-selectid" onChange={onChangehandler} disabled={!isData} value={defaultValue}>
        <option id={OPTION_DEFAULT}>{select + '...'}</option>
        {isData?.map((item) => {
          return (
            <option key={item?.id} id={item?.id?.toString()} selected={isDataExist && item?.name === defaultValue}>
              {item?.name}
            </option>
          );
        })}
