...
const ExistingPremiumBreakUp: React.FC<IExistingPremiumBreakUp> = ({
  ...
  const ExistingPremiumBreakUpBody = (accordionIndex: number) => {
    return (
      <React.Fragment key={accordionIndex}>
        <div className="order-body-content">
          {getPremiumArr()?.sort((a, b) => a.type - b.type)?.map((val: PremiumBreakdown, index) => {
            return (
              <div className="order-body-package-content d-flex w-full justify-content-between" key={index}>
                <div>
          ...
          <div className="order-body-package-content content-light">
            <div>
              {homePolicyRenewal?.coverageType !== null && (
                <div className="subtitle">{homePolicyRenewal?.coverageType}</div>
              )}
            </div>
