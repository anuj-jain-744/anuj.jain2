...
import { taskConstants, toasterTimout, validFileTypes } from "constant";
import "./style.scss";
  ...
  const updateEstimation = async (
    ...
    if (estimationIndex !== -1) {
      ...
      if (field === fields.cause) {
        ...
        if (fileIndex !== -1) {
          ...
          if (file instanceof File === false) {
            ...
            const isDuplicate = currentItem.files.some(
              (fileItem, index) => index !== fileIndex && fileItem.name === file.name && fileItem.name !== ""
            );

            if (isDuplicate) {
              const errorMessage = langData.duplicate_file_error;
              toast.error(errorMessage, {
                autoClose: parseInt(langData.toaster_timeout) || TOAST_AUTOCLOSE_TIMER,
              });
              newFile.error = errorMessage;
      ...
      if (Number.isFinite(siLimit) && Number.isFinite(amount)) {
        ...
        newEstimations[estimationIndex].isLimitExceeded = amount > totalLimit;
      } else newEstimations[estimationIndex].isLimitExceeded = false;
    }
