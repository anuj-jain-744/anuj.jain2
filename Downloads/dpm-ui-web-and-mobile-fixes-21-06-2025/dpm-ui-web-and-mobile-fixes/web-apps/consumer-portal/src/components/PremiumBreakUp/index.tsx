...
const PremiumBreakUp: React.FC<IPremiumBreakUp> = ({
  ...
  const [isAllRendered, setIsAllRendered] = useState<boolean>(true);
  ...
  const premiumBreakUpBody = (title: string) => {
    ...
    // for third party case only

    const filteredTPBenefits = selectedBenefits?.filter((val) => {
      if (isTpOnly && isRenewpolicy && isAllRendered) {
        return val?.benefitPrice === 0; // Filter by benefitPrice === 0 only on the first render
      }
      return true; // Default case: include all benefits
    });


    const benefitsToRender = isTpOnly && isRenewpolicy && isAllRendered
      ? filteredTPBenefits : selectedBenefits;
          ...
          <div className="order-body-content">
            ...
            {selectedBenefits?.filter((value) => value.benefitNameEn)?.map((benefit, index) => (
              <div key={index} className="order-body-package-content content-light">
                <div className="w-50">{benefit.benefitNameEn}</div>
                <div>
