...
export default function LeftSuccess({
  ...
  usePolicyDocuments({ policyNo: policyNum, docType: docType.Policy.Policy_Schedule });

  const { data: invoiceDocuments, error: invoiceError, isError: invoiceIsError, isLoading: isInvoiceLoading, refetch:invoiceRefetch } = 
  usePolicyDocuments({ policyNo: policyNum, docType: docType.Policy.E_Invoice });
  ...
  const policyPeriod = processData.productCode === PRODUCTCODE_TRAVEL && policyData?.startDate && policyData?.expiryDate && langData
    ? formatDate(policyData?.startDate, langData) +
    " - " +
    formatDate(policyData?.expiryDate, langData)
    : extractDate(policyStartDate) +
    " - " +
    extractDate(policyExpiryDate);
      ...
      {!loading && (
        <div className="left-card-container">
          ...
          {claimData && languageData ? (
            <RegisterClaimCard claimData={claimData} langData={languageData} handleDownloadPolicy={handleDownloadPolicy}/>
          ) : data?.isAddon === true ? (
                ...
                {isEndosementPolicy ? (
                  <div className="vehicle-info-container">
                    <Endorsement
                      ...
                      languageData={languageData}
                      endorsementNumber = {issuePolicyData?.model?.endoNo}
                    />
                  </div>
                ) : (
                  <>
