...
import { WORKSHOP, CITY, SELECTED, comprehensiveOD, comprehensiveTP, BankNumDefault, Additional_Remarks, taqderr, lossDate, lossType, OTHERS_CASE_SOURCE_TYPE, TRANSFERRING_COMPENSATION, lossDescription, estAmountOthers, OTHERS_CASE_BANK, OTHERS_TITL, OTHERS_LIABILITY_PER, submit,
} from "../../../../constant";
...
const RegisterClaim = ({
  ...
  const [isIbanBankName, setIbanBankName] = useState<string | undefined>(BankNumDefault);
  ...
  const [isFileUploaded, setFileUploaded] = useState<boolean>(
    (validationData?.estimatedAmount !== null ||
      ...
      claimCheckData?.liability?.length !== 0) && fileData.length > 0
  );
    ...
    switch (value) {
      case "Damage Repair":
       ...
       type === comprehensiveOD && setIbanValid(true);
        if (fileData && fileData[0]?.fileName?.length > 0 && fileData[0] !== null && isIbanValid) {
          setFileUploaded(true);
        } else if (liabilitySelected === 1 && fileData[0] !== null && isIbanValid) {
          setFileUploaded(true);
        } else {
        ...
        if (fileData && fileData[0]?.fileName?.length > 0 && fileData[0] !== null && isIbanValid) {
          ...
          (validationData?.estimatedAmount !== null ||
            ...
            (isAuthenticated &&
              claimCheckData?.liability !== null &&
              isAuthenticated &&
              claimCheckData?.liability?.length > 0)) && isIbanValid
        ) {
          claimsInfo?.SourceType !== OTHERS_CASE_SOURCE_TYPE && setFileUploaded(true);
        } else {
          setFileUploaded(false);
        }
      ...
      case "LiabilityFileUpload":
        claimsInfo?.SourceType !== OTHERS_CASE_SOURCE_TYPE && setLiabilityDocUploaded(true);
        ((claimsInfo?.SourceType !== OTHERS_CASE_SOURCE_TYPE)  && (claimCheckData?.estimatedAmount !== null)) && setEstimatedAmountDataValid(true);
  ...
  //for Loss Type
  const [lossTypeIs, setLossType] = useState<string>(othersClaimInfo?.lossType ? othersClaimInfo?.lossType : "");
    ...
    if (name) {
      switch (name) {
        ...
        case estAmountOthers:
          setEstimatedAmountDataValid(typeof value === 'string' && value.length > 0);
          setEstimatedAmountData(value);
          break;
        // for liability percentage
        case OTHERS_LIABILITY_PER:
          const liabilityPercentageValue = !(!value || isNaN(Number(value))) ? value : ""
  ...
  const contactDetchangeHandler = (
    ...
    switch (name) {
      case "iBAN":
        ...
        (isBankTransferSelected || isDamageRepairSelected || claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE) && setIbanValid(isIBAN);
        break;
      case "mobilenum":
  ...
  const onchangeHandlerFiles = (Filesdata: any) => {
    ...
    if(Filesdata[0] !== null && isIbanValid) {
     ...
     (claimsInfo?.type === comprehensiveTP && Filesdata[0] === null) && setFileUploaded(false);
     comprehensiveTP
  ...
  const submitRegisterClaim = async () => {
    ...
    const payload = {
      referenceNo: validationData?.referenceNo || claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE && claimCheckData?.referenceNo || null,
      sourceType: claimsInfo?.SourceType || null,
      claimRequestType: type,
      caseReportId: claimsInfo?.refNo || null,
      sequenceNo: validationData?.sequenceNo || othersClaimInfo?.isSequenceNo && othersClaimInfo?.SequenceNo || null,
      ...
      compensationType: isBankTransferSelected ? "Bank" : claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE ? OTHERS_CASE_BANK :  "Damage",
      ...
      liabilityPercentage: liabilityPercentage?.length > 0 ? liabilityPercentage : undefined,
      descriptionOfLoss: descriptionOfLoss || null,
      claimSystem: validationData?.claimSystem ||claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE && OTHERS_TITL || null,
      // @Md shamim - once these changes are done, need to remove this piece!!! and bring above line
      //claimSystem: validationData?.claimSystem !== 'IVOX' ? 'EBAO' : validationData?.claimSystem,
      documents: [
      ...(iBanFileData ?? []).map((item: { name: string; base64: string; fileName: string; fileExtension: string; size: number; docFile: string }) => ({
        fileName: item?.fileName ?? null,
        docFile: item?.docFile ?? null,
      })),
      ...(liabilityFileData ?? []).map((item: { name: string; base64: string; fileName: string; fileExtension: string; size: number; docFile: string }) => ({
        fileName: item?.fileName ?? null,
          docFile: item?.docFile ?? null,
      })),
      ]
  ...
  useEffect(() => {
      ...
      if(claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE && type === comprehensiveOD && othersClaimInfo?.isSequenceNo && othersClaimInfo?.SequenceNo && othersClaimInfo?.SequenceNo.length > 0) {
        setIbanValid(true);
      }
      ...
      useEffect(() => {
        if (validationData?.mobileNo && validationData?.mobileNo?.toString()?.length > 0) {
  ...
  const isButtonEnabled =
    ...
    ((isDamageRepairSelected || type === comprehensiveOD
      ? isCitySelected && isCitySelected.length > 0 && isGarageSelectedCount > 0
      : true) ||
      ...
      (isDamageRepairSelected &&
        ...
        isGarageSelectedCount > 0))  || (claimsInfo?.type === comprehensiveOD && (
          isFileUploaded &&
          isMobNumValid && 
          mobilenumData && 
          isIAgree &&
                  ...
                  othersClaimInfo?.SequenceNo.length > 0 ? (
                    <ThemeButton
                      classes={
                        ...
                        isIAgree
                          ? true
                          : false) ||(claimsInfo?.type === comprehensiveTP &&
