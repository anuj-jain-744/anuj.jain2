...
const ClaimDetails = ({
  ...
  const [estAmountUpdate, setEstAmountUpdate] = useState<string | number | null>(null);
  ...
  const changeSelectHandler = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    if (value !== null) {
      ...
      cityData?.map((city) => {
        if (city?.cityNameEn?.toLocaleLowerCase() === value?.toLocaleLowerCase()) {
  ...
  const cityList = (data: City[]) => {
    ...
    if (cityArr?.find((item) => item?.name.toLocaleLowerCase() === validationData?.city?.toLocaleLowerCase())) {
      setSelectedCity(validationData?.city?.toString()?.toUpperCase());
      // update garage data matching with city
      const cityData = data as CityWorkshopRepairList[];
      // map over the data and get the garage list for the selected city
      cityData?.map((city) => {
        if (city?.cityNameEn?.toLocaleLowerCase() === validationData?.city?.toLocaleLowerCase()) {
  ...
  const validateAmount = (value: string,name:string) => {
    ...
    const valueSanitized = (value.length === 1 && value === ".") ? value.replace(/[.]/g, '') : value.trim();
  ...
  const updatedValue = (event: React.FormEvent<HTMLDivElement>) => {
    ...
    else if(name === lossType || name === Data?.description_of_loss || name === OTHERS_LIABILITY_PER ) {
            ...
            <React.Fragment>
                ...
                {((type === comprehensiveOD && selected === 2) || isModal) && (
                  <div className="container uploadAccordion px-4">
                    <UploadDocss
                      ...
                      productType = {Motor?.toLocaleLowerCase()}
                      estimatedAmount = {validationData?.estimatedAmount}
                      liabilityPercentage = {validationData?.liability}
                      caseType = {isNajmCase ? CLAIM_CASES?.NAJM_CASE?.toLocaleLowerCase() : CLAIM_CASES?.POLICE_CASE?.toLocaleLowerCase()}
                    />
                  </div>
                )}
              </div>
                      ...
                      <div className="row d-flex w-100">
                        ...
                        <div className="col-xs-12 col-sm-12 col-md-6 walaa-regular-400">
                          <div className="row d-flex flex-column">
                            ...
                            <div className={`${Array.isArray(selectedGarage) && selectedGarage?.length > 0 ? "show-no-placeholder col" : "col"}`}>
                              <ThemeMultiSelect
              ...
              {(isDamageRepairSelected || type === comprehensiveOD) &&  (
                <div className="px-4 mx-2">
                  ...
                  { (isNajmCase && validationData?.estimatedAmount === null) ? (
                    <React.Fragment>
                     <div className="container uploadAccordion px-4">
                          <UploadDocss
                              fileData={fileData}
              ...
              <div className="row d-flex flex-column w-75">
                ...
                <div className="col walaa-medium-500 estimated-amount-title">
                  {type === comprehensiveTP && (
                    ...
                    onBlurhandler={(event)=>setEstAmountUpdate(formatedAmountToDisplay(event?.target?.value))}
                  />
                  )}
                </div>
              </div>
