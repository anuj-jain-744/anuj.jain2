...
const useFileHandler = ({
  ...
  const handleFileChange = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>, id: string) => {
      ...
      if (!file || !targetItem) {
        toast.error(`${trackClaimInfo?.no_files_selected}`, { autoClose: TOAST_AUTOCLOSE_TIMER || false });
      ...
      if (isDuplicateFileName) {
        ...
        toast.error(errorMessage, { autoClose: TOAST_AUTOCLOSE_TIMER || false });
      ...
      try {
        ...
        toast.error(`${trackClaimInfo?.error_converting_file_to_base_64}`, { autoClose: TOAST_AUTOCLOSE_TIMER || false });
      } finally {
        resetFileInput(fileInput);
      }
    },
