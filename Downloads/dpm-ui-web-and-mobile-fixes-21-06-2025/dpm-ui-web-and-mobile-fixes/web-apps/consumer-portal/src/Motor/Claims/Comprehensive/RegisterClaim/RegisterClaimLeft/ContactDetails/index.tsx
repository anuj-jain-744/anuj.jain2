...
import { SA, IBAN, comprehensiveTP, comprehensiveOD, OTHERS_CASE_SOURCE_TYPE, NOT_VALID_MOBILE, TRAVEL, NATIONAL_ID } from "../../../../../../constant";
...
const ContactDetails = ({
  ...
  const isAuthenticated= useSelector((state: RootState) => state.auth?.isAuthenticated);
    ...
    if(value.includes(" ")){
      ...
      return;
    } else if (event.nativeEvent instanceof KeyboardEvent && event.nativeEvent.code === "Space") {
      event.preventDefault();
      return;
    }
    ...
    if (name === "IBan" || name === "mobilenum" || name === "emailId" || name === languageData?.additional_remarks) {
      if (name === "IBan") {
        ...
        if (sanitizedValue.length > 2 && !sanitizedValue.startsWith(SA)) {
          ...
          if (sanitizedValue.length !== 24) {
            setCompensateError({
              ...compensateError,
              iBan: languageData?.iban_invalid_non_sa || "IBAN not valid for Saudi Accounts",
            });
            changeHandler("iBAN", false);
        ...
        setCompensateData({ ...compensateData, isMobilenum: sanitizedValue.replace(/\D/g, "")});
  ...
  useEffect(() => {
    if (mobilenumData?.toString()?.length > 0) {
      ...
      if(!saudiMobileRegex.test(mobilenumData?.toString())) {
        setCompensateError({
          ...compensateError,
          mobilenum: NOT_VALID_MOBILE || languageData?.phone_no_must_be_starts_wi,
        });
        changeHandler("mobilenum", false);
      }
      else if(saudiMobileRegex.test(mobilenumData?.toString()) && mobilenumData?.toString()?.length === 10) {
        setCompensateError({ ...compensateError, mobilenum: "" });
        setCompensateData({ ...compensateData, isMobilenum: mobilenumData?.toString() });
  ...
  return (
    <div className="px-4 mx-2" data-testid="registerclaimcontact-test">
      <Card className="right-card-register">
        ...
        {((type !== comprehensiveOD)  || (isOthersCase === OTHERS_CASE_SOURCE_TYPE && type === comprehensiveTP) || (isOthersCase === OTHERS_CASE_SOURCE_TYPE && type === comprehensiveOD && !othersClaimInfo?.isSequenceNo))&& (
          <React.Fragment>
                ...
                <div className="d-flex flex-row flex-wrap justify-content-between">
                  <div className="iban-textbox">
                    ...
                    <div className="col pt-2">
                      <InputGroup  onChange={(e) => updatedValue(e as React.FormEvent<HTMLDivElement>)}>
                        <Form.Control
                  ...
                  {((isValidIban && type !== comprehensiveOD) || (isValidIban && (isOthersCase === OTHERS_CASE_SOURCE_TYPE))) && (
                    <>
                      <div className="iban-textbox">
                        <div className="col walaa-regular-400">
                          <TypographyAndIcon
                        ...
                        <ThemeTextbox
                            type={"text"}
                            name="reEnteredIban"
                            placeholder={languageData?.placeholder_enter_iban_num as unknown as string}
                            value={reEnteredIban}
                      ...
                      <div className="iban-textbox pt-3">
                        ...
                        <ThemeTextbox
                            type={"text"}
                            name="bankName"
                            placeholder={languageData?.enter_bank_name as unknown as string}
                            value={bankName}
      ...
      {((isValidIban && type !== comprehensiveOD) || (isValidIban && (isOthersCase === OTHERS_CASE_SOURCE_TYPE))) && (<Iban fileData={fileData} setFileData={setFileData} languageData={languageData || {}} onChequeLeafUpload={() => setChequeLeafUploaded(true)}
                         onChequeLeafRemove={() => setChequeLeafUploaded(false)} />)}

      {((type !== comprehensiveTP && type !== TRAVEL) || (type === comprehensiveTP && isOthersCase === OTHERS_CASE_SOURCE_TYPE)) && (
      <div className="row">
