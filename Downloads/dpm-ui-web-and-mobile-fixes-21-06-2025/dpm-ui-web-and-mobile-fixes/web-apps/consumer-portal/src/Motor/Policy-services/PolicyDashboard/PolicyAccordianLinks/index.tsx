...
import { PRODUCTS_CODE, commonKeywords } from "constant"
import "./index.scss";
...
const getHolderDetails = (policyHolderDetails: PolicyHolderDetails | null, languageData: LanguageData | undefined): HolderDetails => ({
  [languageData?.insurance_name ?? ""]: policyHolderDetails?.customerNameEnglish || policyHolderDetails?.customerNameArabic 
    ? `<li>${capitalizeNameFirstLetter(policyHolderDetails.customerNameEnglish ?? "")}</li><li>${capitalizeNameFirstLetter(policyHolderDetails.customerNameArabic ?? "")}</li>`
    : "",
  ...
  [languageData?.license_type ?? ""]: policyHolderDetails?.licenseType,
