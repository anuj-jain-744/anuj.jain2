...
import {
  INVALID_PRICE_FORMAT_ERROR_MSG,
  CONFIRM_MSG_DELETE,
  POPUP_FOR,
  genderCodes,
...
const TravelerFamily: React.FC<TravelerFamilyProps> = React.memo(
  ({
    ...
    const [invalidSeniorCitizenAge, setInvalidSeniorCitizenAge] = useState<boolean>(false);
    ...
    const MSG = invalidSeniorCitizenAge ? SENIOR_CITIZEN_VALIDATION_MSG : INVALID_FAMILY_TRAVELER_COUNT
    ...
    const handleInputChange = (
      ...
      if (field === "passportExpiryDate" || field === "dateOfBirth") {
        ...
        if (field === "passportExpiryDate") {
          ...
          if (getFullAge(dateText, '/') > 80 && person.type === familtyFlowConstants.TITLES.SR_CITIZEN) {
            setInvalidSeniorCitizenAge(true);
            handleOpenRemoveModal(familyType, person)
          } else { setInvalidSeniorCitizenAge(false) }
          dataRef.current.dobErrors[uiId] = dobCheck === true ? "" : dobCheck;
          if (dobCheck === true) {
    ...
    const handleRemovePopupSubmit = () => {
      ...
      handleDelete(dataRef.current.familyMemberType, dataRef.current.removePerson);
    ...
    return (
      <div className="custom-accordion-container">
        ...
        <ModalPopUp
          ...
          errorMsg={MSG}
          popupUsedFor={invalidSeniorCitizenAge ? familtyFlowConstants.POPUP_USED.SENIOR_CITIZEN_AGE_VALIDATION : INVALID_FAMILY_TRAVELER_COUNT ? familtyFlowConstants.POPUP_USED.LAST_FAMILY_MEMBER_NOT_SELF_DELETE : POPUP_FOR}
