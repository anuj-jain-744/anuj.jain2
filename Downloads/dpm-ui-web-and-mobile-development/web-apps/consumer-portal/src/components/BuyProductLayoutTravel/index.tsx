...
const Layout: React.FC<LayoutProps> = ({
  ...
  const [travelPeriods, setTravelPeriods] = useState<
    Array<Travel.TravelPeriodOption>
  >([]);
  ...
  const {
    makeApiCall: getTravelPeriodList,
    isLoading: travelPeriodLoader,
    errors: travelPeriodError,
    data: travelPeriodData,
  ...
  const [ triggerCoveragePlpan, setTriggerCoveragePlan ] = useState<boolean>(false);
  ...
  const { makeApiCall: redisKey, data: redisData } = useApiCall<{ key: string }, undefined>(
    6,
    `${JAVA_API_ROUTES?.redisGetValue}/${dataRef.current.uniqueKey}`,
    "get",
  );
  ...
  useEffect(() => {
    if (triggerCoveragePlpan === true) {
      handleSelectCoverage()
    }
  }, [triggerCoveragePlpan])
  ...
  useEffect(() => {
    ...
    const primary = {
      uiId: timestamp,
      travellerNameEnglish: toTitleCase(cleanSpaces(ownerData.ownerFullNameEnglish)),
  ...
  useEffect(() => {
    if (travelPeriodLoader === false) {
      if ((travelPeriodError !== null && leftStep === 0) || leftStep === 1) {
        setApiErrorMessage({
          title: travelPeriodError?.name || INTERNAL_SERVER_ERROR,
          description:
            travelPeriodError?.messages?.message_en || SOMETHING_WENT_WRONG,
        });
        setShowAlertModal(true);
      } else if (travelPeriodData) {
        if (
          travelPeriodData.model instanceof Object &&
          Array.isArray(travelPeriodData.model.content)
        ) {
          const items = travelPeriodData.model.content.map(
            (item: Travel.TravelPeriod) => ({
              ...item,
              label: item.codeDesc,
              value: item.codeId,
            })
          );
          setTravelPeriods(items);
        }
      }
    }
  }, [travelPeriodLoader]);
  ...
  useEffect(() => {
    ...
    let disable = true;
    if (selectedPeriod.value && travelStartDate) {
      if (isToggleOn === true || totalCount > 1) disable = false;
    }
    ...
    if (leftStep === 3 || leftStep === 4 || leftStep === 2) {
      ...
      if (
        ...
        const primary = primaryTravelers.map((item) => ({
          ...item,
          travellerNameEnglish: cleanSpaces(item.travellerNameEnglish)
        ...
        const addItems = (
          ...
          persons.forEach((item, index) => {
            ...
            additionalProps.travellerNameEnglish =
              cleanSpaces(additionalProps.travellerNameEnglish);
