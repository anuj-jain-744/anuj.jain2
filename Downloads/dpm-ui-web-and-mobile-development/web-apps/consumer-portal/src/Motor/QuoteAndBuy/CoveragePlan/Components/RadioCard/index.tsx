...
const RadioCard: React.FC<IRadioCard> = ({
  ...
  const { setSelectedBenefits, repairTypeSelected, setRepairTypeSelected, coverageType, isRenewpolicy } = useQuoteAndBuyContext();
  ...
  useEffect(() => {
    ...
    if (isRenewpolicy && coveragePlanSelected && repairTypeSelected) {

      handleOnChange(coveragePlanSelected.toLowerCase());
      setRepairTypeSelected(repairTypeSelected);

    }
  }, [isRenewpolicy, coveragePlanSelected,repairTypeSelected ]);
  ...
  return (
    ...
    >
      ...
      >
        <div className="py-2">
          {languageData && <RadioCardFooter label={radiokey} languageData={languageData} />}
        </div>
      </Card.Footer>
    </Card>
  );
