...
import { comprehensiveOD, OTHERS_CASE_SOURCE_TYPE } from "constant";
    ...
    <div className="register-new-claim-left-card-main d-flex flex-column">
      <div className="register-claim-left-card-top-message-container">
        <p data-testid="registration-modelid" className="walaa-regular-400" onClick={showRegistrationModal}>
          <span className="d-flex float-start walaa-medium-500">{user}</span><p dangerouslySetInnerHTML={{ __html: Data?.register_claim_top_message }} />
        </p>
      </div>
      ...
      {(isBankTransferSelected || isDamageRepairSelected || type === comprehensiveOD || claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE) && (
        <React.Fragment>
          {/* claim details */}
          <div className="register-new-claim-left-card walaa-regular-400">
