...
import { WORKSHOP, CITY, SELECTED, comprehensiveOD, comprehensiveTP, BankNumDefault, Additional_Remarks, taqderr, lossDate, lossType, OTHERS_CASE_SOURCE_TYPE, TRANSFERRING_COMPENSATION, lossDescription, estAmountOthers } from "../../../../constant";
...
const RegisterClaim = ({
  ...
  const onChangeHandler = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    switch (value) {
      ...
      case "LiabilityFileUpload":
        ...
        claimsInfo?.SourceType !== OTHERS_CASE_SOURCE_TYPE && setEstimatedAmountDataValid(true);
        break;
  ...
  const submitRegisterClaim = async () => {
    ...
    const payload = {
      ...
      compensationType: isBankTransferSelected ? "Bank" : claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE ? TRANSFERRING_COMPENSATION :  "Damage",
      ibanNo: isIbanNumData,
      bankName: isIbanBankName,
      dateOfLoss: dateOfLoss || null, 
      tadeerNo: taqdeerValue || null,
      lossType: lossTypeIs || null,
      descriptionOfLoss: descriptionOfLoss || null,
      claimSystem: validationData?.claimSystem || null,
      documents: [
                    ...
                    classes={
                      ...
                      (isDamageRepairSelected || type === comprehensiveOD
                        ...
                        (claimsInfo?.SourceType === OTHERS_CASE_SOURCE_TYPE && 
                          dateOfLoss?.length > 0 && lossTypeIs?.length > 0 &&
                          isIbanValid &&
                          isMobNumValid &&
                          mobilenumData &&
                          isFileUploaded &&
