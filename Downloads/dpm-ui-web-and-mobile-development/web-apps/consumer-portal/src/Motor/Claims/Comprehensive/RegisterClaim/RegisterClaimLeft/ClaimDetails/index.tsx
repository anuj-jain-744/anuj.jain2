...
const ClaimDetails = ({
  ...
  const cityList = (data: City[]) => {
    ...
    if (type === comprehensiveTP && cityArr?.find((item) => item?.name === claimCheckData?.city)) {
      setSelectedCity(claimCheckData?.city?.toString()?.toUpperCase());
      // update garage data matching with city
      const cityData = data as CityWorkshopRepairList[];
      // map over the data and get the garage list for the selected city
      cityData?.map((city) => {
        if (city?.cityNameEn === claimCheckData?.city) {
  ...
  const updatedValue = (event: React.FormEvent<HTMLDivElement>) => {
    ...
    // for taqdeer
      ...
      setTaqdeerError({
        ...taqdeerError,
        taqdeerNo: value?.length > 15 ? NOT_VALID_AMOUNT : "",
      })
    ...
    else if(name === lossType || name === Data?.description_of_loss){
      updateHandler(value, name);
    }
  };
                  ...
                  {type === comprehensiveTP && (
                    ...
                    value={
                      typeof isEstimatedAmountData === "boolean"
                        ? ""
                        : formatNumberForDisplay(isEstimatedAmountData as unknown as string) 
                    }
                    onChangehandler={(event) => {
                      const rawValue = event.target.value.replace(/,/g, ""); 
                      updatedValue({
                        target: { name: "estimatedAmount", value: rawValue },
                      } as React.ChangeEvent<HTMLInputElement>);
                    }}
