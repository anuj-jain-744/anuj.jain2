...
export function getUniquePoliciesWithLatestEndorsementNumber(policies: PolicyDetail[]): PolicyDetail[] {
  const policyMap = new Map<string, PolicyDetail>();

  policies.forEach((policy) => {
    const existingPolicy = policyMap.get(policy.policyNo);

    if (!existingPolicy || new Date(policy.issueDate ?? "") > new Date(existingPolicy.issueDate ?? "")) {
      policyMap.set(policy.policyNo, policy);
    }
  });
