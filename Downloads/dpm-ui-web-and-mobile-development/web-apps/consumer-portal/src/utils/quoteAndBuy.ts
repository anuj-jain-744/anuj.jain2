...
import { Chargeable, genderCodes } from "../constant";
...
export const isValidPassportExpiry = (
  ...
  if (typeof start !== "string") start = start?.toLocaleString();
  let end = expiryDate;
  if (typeof end !== "string") end = end?.toLocaleString();
...
export const getVehicleMakeModel = (vehicleDetailsResponseData: VehicleDetailsResponseData | null) => {
  ...
  const vehilceName = `${vehicleDetailsResponseData?.make ?? ""} ${vehicleDetailsResponseData?.model ?? ""}`;
