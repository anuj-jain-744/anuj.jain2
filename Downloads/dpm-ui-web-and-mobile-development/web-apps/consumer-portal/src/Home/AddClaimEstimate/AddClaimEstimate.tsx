...
const AddClaimEstimate: React.FC<AddClaimEstimateProps> = ({
    ...
    const handleDeleteEstimate = (index: number) => {
        setActiveKey(null);
        setTimeout(() => {
            const updatedEstimates = estimates.filter((_, idx) => idx !== index);
            const updatedFileErrors = fileErrors.filter((_, idx) => idx !== index);
            const updatedAmountErrors = amountErrors.filter((_, idx) => idx !== index);
    ...
    const handleFieldChange = (index: number, field: keyof ClaimEstimate, value: any) => {
        ...
        if (field === "estimateAmount") {
            ...
            const siLimit = selectedCoverage?.siLimit || 0;
            if (Number(value) > siLimit) {
                updatedAmountErrors[index] = `${languageData?.estimated_loss_amount_should_not_exceed} ${siLimit}`;
            } else {
                updatedEstimates[index][field] = value && Number(value);
                updatedAmountErrors[index] = "";
            }
        }
    ...
    const handleFileChange = async (index: number, fileType: keyof ClaimEstimate, file: File | null) => {
        if (file) {
            ...
            const docType = fileType === "policeReport" ? languageData?.police_report :
                fileType === "stolenItemList" ? languageData?.stolen_item_list : languageData?.purchase_invoice;
                    ...
                    return <Accordion.Item eventKey={index.toString()} key={index} className="estimate-accordion-item">
                        <Accordion.Header>
                            <div className="accordion-header-content">
                                ...
                                <img src={Frame} alt="Delete" className="delete-icon" onClick={(e) => { e.stopPropagation(); handleDeleteEstimate(index); }} />
                            ...
                            <div className="estimate-body">
                                <div className="row mb-3">
                                    ...
                                    <div className="col-md-6">
                                        ...
                                        />
                                        {amountErrors[index] && (
                                            <div className="file-error-message">{getCurrencySymbol(amountErrors[index])}</div>
                                        )}
                                    </div>
                                </div>
