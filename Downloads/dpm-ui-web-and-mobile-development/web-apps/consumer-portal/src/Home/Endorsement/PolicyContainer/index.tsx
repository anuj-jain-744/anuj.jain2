...
import { apiRoutes, JAVA_API_ROUTES, PORTAL , PaymentUrl} from "constant";
...
const PolicyContainer: FC<PolicyCardProps> = ({ policyData, languageData, navigateTo }) => {
  ...
  **/
  ...
  const benefitsToEndorsement = (data: EndorsmentPolicyRisk[]) => {
    ...
    let newlyAddedbenefits = [];
    for (let element in policyCoverage) {
  ...
  const checkBenefitsToAdd = (code: string, isPurchasedBenefit: number) => {
    const action = isPurchasedBenefit === -1 ? ADD : DELETE;
  ...
  const handleClickPayment = () => {
    ...
    redisCall({ key: policyDetails?.endorsementNo, value: null });
  }
    ...
    <div className="home-claim-details">
      ...
      <OTPWrapper
        ...
        handleSuccessValidation={async () => {
          ...
          const encryptedQuoteNumber = btoa(`${policyDetails?.endorsementNo}_02`);
