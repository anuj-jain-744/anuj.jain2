...
const RegisterClaim: FC<PolicyCardProps> = ({ claimData, languageData, backBtnClickHandler, isPolicyCardSelected, policies }) => {
  ...
  useEffect(() => {
    if (nonMotorRegistrationData) {
      ...
      const address = displayHouseAddress(viewPolicy?.policyCustomer[0]?.primaryAddress, currentLanguage, ar);
      const claimData = {
  ...
  const isPaymentDisabled = () => {
    return isValid && nonMotorFNOL && isTermCondition && estimatedValues.length  //isTermCondition && nonMotorFNOL;
  }
