...
const RegisterClaimLeft: FC<PolicyCardProps> = ({
  ...
  const { selectedPolicyNumber, viewPolicy, isTermCondition } = policyClaim;
  const { currentLanguage } = useCommonContext();
  const { ar } = commonKeywords;
  const [dropdownItems, setDropdownItems] = useState<[]>([]);
  const [isRefNoModal, setRefNoShow] = useState<boolean>(false);
  const [policycustomer, setPoicyCustomer] = useState<{ [key: string]: string } | null>({} || null)
  useEffect(() => {
  ...
  const policyInfo = languageData && languageData['we_have_retrieved_all_your_policy_related_information.'];
  ...
  return (
    <div className="policy-claim-left-card">
      ...
      <div className="home-claim-content">
        {viewPolicy ?
          <>
            ...
            {ar === currentLanguage ? viewPolicy?.policyCustomer[0]?.customerNameArabic : viewPolicy?.policyCustomer[0]?.customerNameEnglish},
            {` ${policyInfoData[0]}`}
      ...
      {viewPolicy && nonMotorFNOL && !claimFNOLError && <div className="policy-claim-container">
        <div className="select-policy-header walaa-medium-500">
      ...
      {viewPolicy && nonMotorFNOL && !claimFNOLError &&
        <TermsAndCon
          languageData={languageData}
