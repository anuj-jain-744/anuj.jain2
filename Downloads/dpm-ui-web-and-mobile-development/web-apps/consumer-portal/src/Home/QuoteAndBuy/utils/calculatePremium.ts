...
export const calculatePremium = (
  ...
  let premiumFinalPrices: [] = [];

  premiumKeys.forEach((data) => {
    const pricingOptions: CalculatePremiumApiResponse = premiumResponse[data]?.pricingOptions;
    if (pricingOptions && pricingOptions?.length > 0) {
      const price = pricingOptions[0]?.finalAmount as never;
