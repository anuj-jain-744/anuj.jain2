...
describe('useCancelRefund', () => {
  ...
  it('should set loading state when fetchCancelRefundData is called', async () => {
    const { result } = renderHook(() => useCancelRefund({ PolicyNo: 'P-OS01-25-605-000026' }));
  ...
  it('should handle successful API response', async () => {
    const mockResponse = {
      code: 1,
      message: 'SUCCESS',
      data: { policyNo: 'P-OS01-25-605-000026' },
    };

    mockCallAPI.mockResolvedValueOnce(mockResponse);

    const { result, waitForNextUpdate } = renderHook(() => useCancelRefund({ PolicyNo: 'P-OS01-25-605-000026' }));
