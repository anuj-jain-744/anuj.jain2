...
const ProductElement: React.FC<ProductElementProps> = ({ 
    ...
    return (
        <div className={`${style.container} ${isZeroUserValid ? style.noCursor : ''}`}>
            <div
            ...
            >
                <img src={isZeroUserValid ? getIconZeroUser(iconName) : getIcon(iconName)} alt={`${iconName}Icon`}
                 className={isZeroUserValid  ? style.iconBoxDisabled :  style.iconBox} />
                <div className={isZeroUserValid ? style.productDisableText : 
                    style.productText}>{text}</div>
            </div>
            {(showMenu && !isDisabled) && 
