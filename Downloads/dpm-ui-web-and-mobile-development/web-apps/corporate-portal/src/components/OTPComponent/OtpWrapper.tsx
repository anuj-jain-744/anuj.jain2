...
// Constants
import { INVALID_OTP_ERROR_CODE, RESEND_OTP_ERROR_CODE, OTP_TIMER, INVALID_MOBILE_ERROR_CODE, passwordInput } from "constant";
...
export const OTPWrapper: React.FC<OTPWrapperProps> = ({
  ...
  const [resendOTPBtnClicked, setResendOTPBtnClicked] = useState<number>(0);
  const [resendOTPBtnDisabled, setResendOTPBtnDisabled] = useState<boolean>(false);
  ...
  const validateOtp = async () => {
    ...
    if (LoginContext?.navigateToErrorFrom === 0) {
      payload = {
        ...payload,
        "type": passwordInput.login,
      }
    }
    if (LoginContext?.navigateToErrorFrom === 6) {
      payload = {
        ...payload,
        "type": passwordInput.RESETPASSWORD,
        "encryptedPassword": LoginContext?.encryptedPassword,
      }
    }
