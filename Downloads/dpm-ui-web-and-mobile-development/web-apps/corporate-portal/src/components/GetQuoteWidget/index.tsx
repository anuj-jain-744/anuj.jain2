...
import { isCrossed48Hours, Najm_Case } from "./ClaimTypeUtil";
...
// handle class name on dropdown
  ...
  const handleResend = (resendSessionId: string) => {
    if (customTheme) {
      resendOTP = true;
      fetchInitiateClaimAPI();
    }
  ...
  const fetchInitiateClaimAPI = async () => {
    ...
    try {
      ...
      if (response.message === "SUCCESS" || response.code === 1) {
        ...
        if (response?.errors && response?.errors[0]?.code === sequenceNumberErrorCode) {
          setMissingData(true);
          setShow(true);
        } else if (response?.errors && response?.errors[0]?.code === retrieveCaseErrorCode) {
        ...
        //Error code DTXJCRC4003 provided by Java team - Need to consider this Code for OTHER case redirect
        else if (response?.errors[0]?.code == othersCaseErrorCode) {
          // check for najm case
          ...
          // if not najm, then it could be police(as no time frame defined in police, can take to other scenario ) or other case
          else {
            //setting source type specific to other case
            setClaimsInfo({
              refNo:
  ...
  return (
    <>
      ...
      />
      {(otpInfo || languageOTPData) && showOTPModal && (
        <OTPValidation
