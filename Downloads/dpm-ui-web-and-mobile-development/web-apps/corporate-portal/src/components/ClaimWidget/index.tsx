...
export const ClaimWidget: React.FC<ClaimWidgetProps> = ({ formHeaderData, navigateTo }) => {
  ...
  const generateClaimTrackOtp = async () => {
    ...
    const urlRequestBody: GenerateOTPPayload = {
      ...
      const requestBody: GenerateOTPPayload = {
        ...
        ownerID: userId ? userId : formInput.national_id_iqama_id_corp_id,
  ...
  useEffect(() => {
    if (responseData) {
      ...
      if (!isAuthenticated) {
        ...
        navigateTo?.(`/${camelCase(responseData?.productName?.split(' ')[0]) ?? 'Motor'}/Claim/Track-Your-Claim`, {
  ...
  useEffect(() => {
    if (validateData) {
      ...
      navigateTo?.(`/${camelCase(validateData?.productName?.split(' ')[0]) ?? 'Motor'}/Claim/Track-Your-Claim`, { trackClaimData: validateData });
      // setting data here to pass to next page
