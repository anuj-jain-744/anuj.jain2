...
import StaticSideBarCard from "../../../../consumer-portal/src/components/StaticSideBarCard";
import {BlueFormFooter} from "../../../../consumer-portal/src/components/BlueFormFooter";
...
export function ContactWalaaScreen({ sitedata }: ContactWalaaProps) {
  ...
  const checkIsbuttonDisabled = useCallback(() => {
    ...
    const mandatoryFieldValues = mandatoryFields.map((val: formFieldData) => {
      if (
        val?.field_type === "radios" &&
        formInput[val?.field_name] === undefined
      ) {
        return false;
      }
  ...
  const handleSubmit = () => {
    ...
    // onSubmitHandle({data:"hello"});
    navigate('/contact-walaa/success')
  };
  ...
  return (
    <>
                  ...
                  {formFields.length > 0 &&
                    formFields.map((val, index) => (
                      <React.Fragment key={index}>
                        {isColumnVisible(val?.field_validation, formInput) && (
                          <Col
                            xl={
      ...
      {isAuthenticated && <BlueFormFooter submitClickHandler={handleSubmit} backBtnClickHandler={goBack} isDisabled={!checkIsbuttonDisabled()}/>}
      {currentTab === 1 && !isAuthenticated && (
