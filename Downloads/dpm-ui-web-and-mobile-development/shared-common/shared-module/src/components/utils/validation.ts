...
export const validateIBAN = (iban: string) => {
  ...
  (char: string): string => (char.charCodeAt(0) - 55).toString()
...
export const validateIbanNonSA = (inputString: string) => {
...
export const formatFileSize = (size: number) => {
  ...
  const fixedSize: string = (Number((size / Math.pow(1024, i)).toFixed(2)) * 1).toString();
